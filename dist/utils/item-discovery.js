...
export async function getActiveChangeIds(root = process.cwd()) {
    const changesPath = path.join(root, 'ovsespec', 'changes');
    try {
...
export async function getSpecIds(root = process.cwd()) {
    const specsPath = path.join(root, 'ovsespec', 'specs');
    const result = [];
    try {
...
export async function getArchivedChangeIds(root = process.cwd()) {
    const archivePath = path.join(root, 'ovsespec', 'changes', 'archive');
    try {
