...
/**
 * Returns the list of available change directory names under ovsespec/changes/.
 * Excludes the archive directory and hidden directories.
 */
export async function getAvailableChanges(projectRoot) {
    const changesPath = path.join(projectRoot, 'ovsespec', 'changes');
    try {
...
export async function validateChangeExists(changeName, projectRoot) {
    if (!changeName) {
        ...
        if (available.length === 0) {
            throw new Error('No changes found. Create one with: ovsespec new change <name>');
        }
    ...
    const changePath = path.join(projectRoot, 'ovsespec', 'changes', changeName);
    const exists = fs.existsSync(changePath) && fs.statSync(changePath).isDirectory();
    if (!exists) {
        const available = await getAvailableChanges(projectRoot);
        if (available.length === 0) {
            throw new Error(`Change '${changeName}' not found. No changes exist. Create one with: ovsespec new change <name>`);
        }
