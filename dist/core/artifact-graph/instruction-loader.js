 ...
 * 2. Schema from .ovsespec.yaml metadata (if exists in change directory)
...
export function loadChangeContext(projectRoot, changeName, schemaName) {
    const changeDir = FileSystemUtils.canonicalizeExistingPath(path.join(projectRoot, 'ovsespec', 'changes', changeName));
