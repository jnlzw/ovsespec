...
保留原命令意图：检查变更是否适合结束，必要时提示同步 specs，然后把 active change 移动到 archive。团队版增强只是在归档前增加团队就绪检查和交接摘要，不改变“归档变更”的核心职责。

**输入**：可以指定变更名，例如 \`/ovsx:archive add-auth\`。如果省略，先尝试从上下文推断；如果模糊，必须让用户选择。

**团队协作增强**

- 归档前检查是否还有未完成任务、未同步 specs、未确认 owner/reviewer、跨团队交接、发布/迁移/回滚事项。
...
4. **评估 delta spec 同步状态**
   ...
   - 判断会产生哪些新增、修改、移除、重命名。
   - 汇总影响 capability、owner/reviewer、消费者、契约或发布风险。
   - 在询问前展示合并摘要。
...
6. **展示归档摘要**
   ...
   - 任务和产物完成情况
   - 如适用，团队交接摘要：影响仓库/模块、owner/reviewer、剩余风险、发布/迁移/回滚事项
...
- 归档摘要必须清楚说明 specs、任务、产物和团队交接状态。`;
export function getArchiveChangeSkillTemplate() {
    return {
        name: 'ovsespec-archive-change',
        description: '归档已完成的 OvseSpec 变更；团队场景会检查 specs、任务、owner、交接和发布风险。',
...
export function getOvsxArchiveCommandTemplate() {
    return {
        name: 'OVSX: Archive',
        description: '归档已完成变更，并输出 specs、任务、团队交接和风险状态',
        category: 'Workflow',
        tags: ['workflow', 'archive', 'team'],
