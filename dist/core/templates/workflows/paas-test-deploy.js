const PAAS_TEST_DEPLOY_INSTRUCTIONS = `执行一次团队版 PaaS test deploy 流程。

这个 workflow 用于把某个已实现或准备验证的 change 部署到 test/staging 类环境，并完成部署前检查、发起部署、验证、回滚准备和团队交接。它不是 production deploy 命令；除非用户明确说明并确认，否则不要对生产环境执行任何操作。

**输入**：\`/ovsx:paas-test-deploy\` 后面可以包含 change 名、服务名、模块名、环境名、branch、commit、image tag 或用户的自然语言部署目标。

**团队协作增强**

- 明确 deploy target：workspace、repo、service/app、module、environment、branch/commit、artifact/image、owner、reviewer、tester。
...
- 对配置变更、数据迁移、接口契约、feature flag、灰度规则、回滚路径和通知对象给出清晰摘要。
...
1. **确认 deploy scope**
   ...
   - owner/reviewer/tester 分别是谁？

   如果用户要求 production deploy，必须明确二次确认，并把风险、rollback、approval 要求列出来。
...
3. **部署前检查**
   ...
   - 如果存在未提交改动、未完成 tasks、缺少 reviewer 或目标环境不明确，暂停并让用户确认。
...
5. **执行 test deploy**
   ...
   **重要**：
   ...
   - 如果需要 secret、token、approval、ticket 或 release window，不要绕过，明确说明阻塞点。
...
**Owner:** <owner>
**Reviewer/Tester:** <reviewer/tester>
...
- 不要猜 service、environment、cluster、namespace、image tag、owner 或 reviewer。
...
export function getPaasTestDeploySkillTemplate() {
    return {
        ...
        description: '执行团队版 PaaS test deploy 流程，覆盖部署前检查、部署确认、验证、rollback 和交接。',
...
export function getOvsxPaasTestDeployCommandTemplate() {
    return {
        ...
        description: '执行团队版 PaaS test deploy，并输出部署、验证、rollback 和交接摘要',
        category: 'Workflow',
        tags: ['workflow', 'paas', 'deploy', 'team'],
