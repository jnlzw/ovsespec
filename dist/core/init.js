 ...
 * Init Command
 *
 * Sets up OvseSpec with Agent Skills and /ovsx:* slash commands.
...
import { AI_TOOLS, OVSESPEC_DIR_NAME, } from './config.js';
...
const { version: OVSESPEC_VERSION } = require('../../package.json');
// -----------------------------------------------------------------------------
// Constants
// -----------------------------------------------------------------------------
...
const WORKFLOW_TO_SKILL_DIR = {
    'explore': 'ovsespec-explore',
    'new': 'ovsespec-new-change',
    'continue': 'ovsespec-continue-change',
    'apply': 'ovsespec-apply-change',
    'ff': 'ovsespec-ff-change',
    'sync': 'ovsespec-sync-specs',
    'archive': 'ovsespec-archive-change',
    'bulk-archive': 'ovsespec-bulk-archive-change',
    'verify': 'ovsespec-verify-change',
    'onboard': 'ovsespec-onboard',
    'propose': 'ovsespec-propose',
    'paas-test-deploy': 'ovsespec-paas-test-deploy',
};
// -----------------------------------------------------------------------------
// Init Command Class
// -----------------------------------------------------------------------------
export class InitCommand {
    ...
    async execute(targetPath) {
        ...
        const ovsespecDir = OVSESPEC_DIR_NAME;
        const ovsespecPath = path.join(projectPath, ovsespecDir);
        // Validation happens silently in the background
        const extendMode = await this.validate(projectPath, ovsespecPath);
        ...
        await this.createDirectoryStructure(ovsespecPath, extendMode);
        // Generate skills and commands for each tool
        const results = await this.generateSkillsAndCommands(projectPath, validatedTools);
        // Create config.yaml if needed
        const configStatus = await this.createConfig(ovsespecPath, extendMode);
    ...
    // VALIDATION & SETUP
    // ═══════════════════════════════════════════════════════════
    async validate(projectPath, ovsespecPath) {
        const extendMode = await FileSystemUtils.directoryExists(ovsespecPath);
    ...
    async handleLegacyCleanup(projectPath, extendMode) {
        ...
        if (this.force || !canPrompt) {
            ...
            // Legacy slash commands are 100% OvseSpec-managed, and config file cleanup
    ...
    async getSelectedTools(toolStates, extendMode, detectedTools, projectPath) {
        ...
        if (configuredNames.length > 0) {
            console.log(`OvseSpec configured: ${configuredNames.join(', ')} (pre-selected)`);
        }
    ...
    // DIRECTORY STRUCTURE
    // ═══════════════════════════════════════════════════════════
    async createDirectoryStructure(ovsespecPath, extendMode) {
        if (extendMode) {
            // In extend mode, just ensure directories exist without spinner
            const directories = [
                ovsespecPath,
                path.join(ovsespecPath, 'specs'),
                path.join(ovsespecPath, 'changes'),
                path.join(ovsespecPath, 'changes', 'archive'),
            ];
        ...
        const spinner = this.startSpinner('Creating OvseSpec structure...');
        const directories = [
            ovsespecPath,
            path.join(ovsespecPath, 'specs'),
            path.join(ovsespecPath, 'changes'),
        ...
        spinner.stopAndPersist({
            ...
            text: PALETTE.white('OvseSpec structure created'),
        });
    }
    // ═══════════════════════════════════════════════════════════
    ...
    async generateSkillsAndCommands(projectPath, tools) {
        ...
        const delivery = globalConfig.delivery ?? 'skills';
            ...
            try {
                ...
                if (shouldGenerateSkills) {
                    ...
                    for (const { template, dirName } of skillTemplates) {
                        ...
                        const skillContent = generateSkillContent(template, OVSESPEC_VERSION, transformer);
    ...
    // CONFIG FILE
    // ═══════════════════════════════════════════════════════════
    async createConfig(ovsespecPath, extendMode) {
        const configPath = path.join(ovsespecPath, 'config.yaml');
        const configYmlPath = path.join(ovsespecPath, 'config.yml');
    ...
    displaySuccessMessage(projectPath, tools, results, configStatus) {
        console.log();
        console.log(chalk.bold('OvseSpec Setup Complete'));
        console.log();
        ...
        if (configStatus === 'created') {
            console.log(`Config: ovsespec/config.yaml (schema: ${DEFAULT_SCHEMA})`);
        }
        else if (configStatus === 'exists') {
            // Show actual filename (config.yaml or config.yml)
            const configYaml = path.join(projectPath, OVSESPEC_DIR_NAME, 'config.yaml');
            const configYml = path.join(projectPath, OVSESPEC_DIR_NAME, 'config.yml');
            const configName = fs.existsSync(configYaml) ? 'config.yaml' : fs.existsSync(configYml) ? 'config.yml' : 'config.yaml';
            console.log(`Config: ovsespec/${configName} (exists)`);
        }
        else {
        ...
        const activeDelivery = globalCfg.delivery ?? 'skills';
