...
export async function writeUpdatedSpec(update, rebuilt, counts) {
    ...
    const specName = path.basename(path.dirname(update.target));
    console.log(`Applying changes to ovsespec/specs/${specName}/spec.md:`);
    if (counts.added)
...
export async function applySpecs(projectRoot, changeName, options = {}) {
    const changeDir = path.join(projectRoot, 'ovsespec', 'changes', changeName);
    const mainSpecsDir = path.join(projectRoot, 'ovsespec', 'specs');
    ...
    for (const p of prepared) {
        ...
        if (!options.dryRun) {
            ...
            if (!options.silent) {
                console.log(`Applying changes to ovsespec/specs/${capability}/spec.md:`);
                if (p.counts.added)
            ...
            console.log(`Would apply changes to ovsespec/specs/${capability}/spec.md:`);
            if (p.counts.added)
                console.log(`  + ${p.counts.added} added`);
            if (p.counts.modified)
                console.log(`  ~ ${p.counts.modified} modified`);
