 ...
 * Update Command
 *
 * Refreshes OvseSpec skills and commands for configured tools.
...
export class UpdateCommand {
    ...
    async execute(projectPath) {
        ...
        const ovsespecPath = path.join(resolvedProjectPath, OVSESPEC_DIR_NAME);
        // 1. Check ovsespec directory exists
        if (!await FileSystemUtils.directoryExists(ovsespecPath)) {
            throw new Error(`No OvseSpec directory found. Run 'ovsespec init' first.`);
        }
        // 2. Perform one-time migration if needed before any legacy upgrade generation.
        // Use detected tool directories to preserve existing ovsx skills/commands.
        ...
        if (configuredTools.length === 0 && newlyConfiguredTools.length === 0) {
            ...
            console.log(chalk.dim('Run "ovsespec init" to set up tools.'));
            return;
        }
        ...
        const toolStatuses = configuredTools.map((toolId) => {
            const status = getToolVersionStatus(resolvedProjectPath, toolId, OVSESPEC_VERSION);
        ...
        if (updatedTools.length > 0) {
            console.log(chalk.green(`✓ Updated: ${updatedTools.join(', ')} (v${OVSESPEC_VERSION})`));
        }
    ...
    displayUpToDateMessage(toolStatuses) {
        ...
        console.log(chalk.green(`✓ All ${toolStatuses.length} tool(s) up to date (v${OVSESPEC_VERSION})`));
    ...
    displayUpdatePlan(toolsToUpdate, statusByTool, upToDate) {
        const updates = toolsToUpdate.map((toolId) => {
            ...
            if (status?.needsUpdate) {
                ...
                return `${status.toolId} (${fromVersion} → ${OVSESPEC_VERSION})`;
            }
    ...
    detectNewTools(projectPath, configuredTools) {
        ...
        if (newTools.length > 0) {
            ...
            console.log();
            console.log(chalk.yellow(`Detected new ${toolNoun}: ${newToolNames.join(', ')}. Run 'ovsespec init' to add ${pronoun}.`));
        }
    }
    /**
    ...
    displayExtraWorkflowsNote(projectPath, configuredTools, profileWorkflows) {
        ...
        if (extraWorkflows.length > 0) {
            console.log(chalk.dim(`Note: ${extraWorkflows.length} extra workflows not in profile (use \`ovsespec config profile\` to manage)`));
        }
    }
    /**
    ...
    displayOldCoreCustomProfileNote(profile, workflows) {
        ...
        console.log(chalk.dim('Run `ovsespec config profile core` and then `ovsespec update` to add sync.'));
    }
    /**
    ...
    /**
     * Detect and handle legacy OvseSpec artifacts.
