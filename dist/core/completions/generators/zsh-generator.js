...
/**
 * Generates Zsh completion scripts for the OvseSpec CLI.
 * Follows Zsh completion system conventions using the _ovsespec function.
 */
export class ZshGenerator {
    ...
    generate(commands) {
        ...
        for (const cmd of commands) {
            ...
            commandCaseLines.push(`          _ovsespec_${this.sanitizeFunctionName(cmd.name)}`);
        ...
        return `#compdef ovsespec

# Zsh completion script for OvseSpec CLI
...
${commandList}
  ...
  case $state in
    command)
      _describe "ovsespec command" commands
      ;;
    args)
      case $words[1] in
${commandCases}
...
`;
    ...
    generateCommandFunction(cmd) {
        const funcName = `_ovsespec_${this.sanitizeFunctionName(cmd.name)}`;
        const lines = [];
        lines.push(`${funcName}() {`);
        ...
        if (cmd.subcommands && cmd.subcommands.length > 0) {
            ...
            for (const subcmd of cmd.subcommands) {
                ...
                lines.push(`          _ovsespec_${this.sanitizeFunctionName(cmd.name)}_${this.sanitizeFunctionName(subcmd.name)}`);
                lines.push('          ;;');
            }
            lines.push('      esac');
            lines.push('      ;;');
    ...
    generateSubcommandFunction(parentName, subcmd) {
        const funcName = `_ovsespec_${this.sanitizeFunctionName(parentName)}_${this.sanitizeFunctionName(subcmd.name)}`;
        const lines = [];
        lines.push(`${funcName}() {`);
        lines.push('  _arguments \\');
        // Add flags
    ...
    generatePositionalSpec(positionalType) {
        switch (positionalType) {
            case 'change-id':
                return "'*: :_ovsespec_complete_changes'";
            case 'spec-id':
                return "'*: :_ovsespec_complete_specs'";
            case 'change-or-spec-id':
                return "'*: :_ovsespec_complete_items'";
            case 'schema-name':
                return "'*: :_ovsespec_complete_schemas'";
            case 'path':
                return "'*:path:_files'";
            case 'shell':
    ...
    generateIndexedPositionalSpec(positional, index) {
        ...
        switch (positional.type) {
            case 'change-id':
                return `'${index}${separator}${name}:_ovsespec_complete_changes'`;
            case 'spec-id':
                return `'${index}${separator}${name}:_ovsespec_complete_specs'`;
            case 'change-or-spec-id':
                return `'${index}${separator}${name}:_ovsespec_complete_items'`;
            case 'schema-name':
                return `'${index}${separator}${name}:_ovsespec_complete_schemas'`;
            case 'path':
