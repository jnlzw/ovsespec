...
/**
 * Generates PowerShell completion scripts for the OvseSpec CLI.
...
export class PowerShellGenerator {
    ...
    generate(commands) {
        ...
        return `# PowerShell completion script for OvseSpec CLI
...
`;
    ...
    generatePositionalCompletion(positionalType, indent) {
        ...
        switch (positionalType) {
            case 'change-id':
                lines.push(`${indent}Get-OvseSpecChanges | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {`);
                lines.push(`${indent}    [System.Management.Automation.CompletionResult]::new($_, $_, "ParameterValue", "Change: $_")`);
                lines.push(`${indent}}`);
                break;
            case 'spec-id':
                lines.push(`${indent}Get-OvseSpecSpecs | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {`);
                lines.push(`${indent}    [System.Management.Automation.CompletionResult]::new($_, $_, "ParameterValue", "Spec: $_")`);
                lines.push(`${indent}}`);
                break;
            case 'change-or-spec-id':
                lines.push(`${indent}$items = @(Get-OvseSpecChanges) + @(Get-OvseSpecSpecs)`);
                ...
                break;
            case 'schema-name':
                lines.push(`${indent}Get-OvseSpecSchemas | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {`);
