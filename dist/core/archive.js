...
export class ArchiveCommand {
    async execute(changeName, options = {}) {
        const targetPath = '.';
        const changesDir = path.join(targetPath, 'ovsespec', 'changes');
        const archiveDir = path.join(changesDir, 'archive');
        const mainSpecsDir = path.join(targetPath, 'ovsespec', 'specs');
        // Check if changes directory exists
        try {
            await fs.access(changesDir);
        }
        catch {
            throw new Error("No OvseSpec changes directory found. Run 'ovsespec init' first.");
        }
