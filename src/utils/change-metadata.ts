...
/**
 * Writes change metadata to .ovsespec.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .ovsespec.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .ovsespec.yaml metadata (if exists)
 * 3. Schema from ovsespec/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/ovsespec/changes/change-name)
