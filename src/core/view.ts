...
export class ViewCommand {
  async execute(targetPath: string = '.'): Promise<void> {
    const ovsespecDir = path.join(targetPath, 'ovsespec');
    
    if (!fs.existsSync(ovsespecDir)) {
      console.error(chalk.red('No ovsespec directory found'));
      process.exit(1);
    }
    ...
    const changesData = await this.getChangesData(ovsespecDir);
    const specsData = await this.getSpecsData(ovsespecDir);
    ...
    console.log('\n' + '═'.repeat(60));
    console.log(chalk.dim(`\nUse ${chalk.white('ovsespec list --changes')} or ${chalk.white('ovsespec list --specs')} for detailed views`));
  }

  private async getChangesData(ovsespecDir: string): Promise<{
    draft: Array<{ name: string }>;
    active: Array<{ name: string; progress: { total: number; completed: number } }>;
    completed: Array<{ name: string }>;
  }> {
    const changesDir = path.join(ovsespecDir, 'changes');
  ...
  private async getSpecsData(ovsespecDir: string): Promise<Array<{ name: string; requirementCount: number }>> {
    const specsDir = path.join(ovsespecDir, 'specs');
