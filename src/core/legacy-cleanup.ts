/**
 * Legacy cleanup module for detecting and removing OvseSpec artifacts
...
import { OVSESPEC_MARKERS } from './config.js';

/**
 * Legacy config file names from the old ToolRegistry.
 * These were config files created at project root with OvseSpec markers.
 */
export const LEGACY_CONFIG_FILES = [
  ...
  'QODER.md',
  'IFLOW.md',
  'AGENTS.md', // root AGENTS.md (not ovsespec/AGENTS.md)
  'QWEN.md',
] as const;
...
export const LEGACY_SLASH_COMMAND_PATHS: Record<string, LegacySlashCommandPattern> = {
  // Directory-based: .tooldir/commands/ovsespec/ or .tooldir/commands/ovsespec/*.md
  'claude': { type: 'directory', path: '.claude/commands/ovsespec' },
  'codebuddy': { type: 'directory', path: '.codebuddy/commands/ovsespec' },
  'qoder': { type: 'directory', path: '.qoder/commands/ovsespec' },
  'lingma': { type: 'directory', path: '.lingma/commands/ovsespec' },
  'crush': { type: 'directory', path: '.crush/commands/ovsespec' },
  'gemini': { type: 'directory', path: '.gemini/commands/ovsespec' },
  'costrict': { type: 'directory', path: '.cospec/ovsespec/commands' },

  // File-based: individual ovsespec-*.md files in a commands/workflows/prompts folder
  'cursor': { type: 'files', pattern: '.cursor/commands/ovsespec-*.md' },
  'windsurf': { type: 'files', pattern: '.windsurf/workflows/ovsespec-*.md' },
  'kilocode': { type: 'files', pattern: '.kilocode/workflows/ovsespec-*.md' },
  'kiro': { type: 'files', pattern: '.kiro/prompts/ovsespec-*.prompt.md' },
  'github-copilot': { type: 'files', pattern: '.github/prompts/ovsespec-*.prompt.md' },
  'amazon-q': { type: 'files', pattern: '.amazonq/prompts/ovsespec-*.md' },
  'cline': { type: 'files', pattern: '.clinerules/workflows/ovsespec-*.md' },
  'roocode': { type: 'files', pattern: '.roo/commands/ovsespec-*.md' },
  'auggie': { type: 'files', pattern: '.augment/commands/ovsespec-*.md' },
  'factory': { type: 'files', pattern: '.factory/commands/ovsespec-*.md' },
  'opencode': { type: 'files', pattern: ['.opencode/command/ovsx-*.md', '.opencode/command/ovsespec-*.md'] },
  'continue': { type: 'files', pattern: '.continue/prompts/ovsespec-*.prompt' },
  'antigravity': { type: 'files', pattern: '.agent/workflows/ovsespec-*.md' },
  'iflow': { type: 'files', pattern: '.iflow/commands/ovsespec-*.md' },
  'junie': { type: 'files', pattern: ['.junie/commands/ovsx-*.md', '.junie/commands/ovsespec-*.md'] },
  'qwen': { type: 'files', pattern: '.qwen/commands/ovsespec-*.toml' },
  'codex': { type: 'files', pattern: '.codex/prompts/ovsespec-*.md' },
};
...
export interface LegacyDetectionResult {
  /** Config files with OvseSpec markers detected */
  configFiles: string[];
  ...
  /** Whether ovsespec/AGENTS.md exists */
  hasOpenspecAgents: boolean;
  /** Whether ovsespec/project.md exists (preserved, migration hint only) */
  hasProjectMd: boolean;
  /** Whether root AGENTS.md has OvseSpec markers */
...
/**
 * Detects all legacy OvseSpec artifacts in a project.
 *
...
/**
 * Detects legacy config files with OvseSpec markers.
 ...
 *
 * @param projectPath - The root path of the project
 * @param pattern - Glob pattern like '.cursor/commands/ovsespec-*.md'
...
async function findLegacySlashCommandFiles(
  ...
  try {
    ...
    // ovsespec-*.md -> /^ovsespec-.*\.md$/
    // ovsespec-*.prompt.md -> /^ovsespec-.*\.prompt\.md$/
    // ovsespec-*.toml -> /^ovsespec-.*\.toml$/
...
/**
 * Detects legacy OvseSpec structure files (AGENTS.md and project.md).
 *
 * @param projectPath - The root path of the project
...
export async function detectLegacyStructureFiles(
  ...
  // Check for ovsespec/AGENTS.md
  const ovsespecAgentsPath = FileSystemUtils.joinPath(projectPath, 'ovsespec', 'AGENTS.md');
  hasOpenspecAgents = await FileSystemUtils.fileExists(ovsespecAgentsPath);

  // Check for ovsespec/project.md (for migration messaging, not deleted)
  const projectMdPath = FileSystemUtils.joinPath(projectPath, 'ovsespec', 'project.md');
  hasProjectMd = await FileSystemUtils.fileExists(projectMdPath);

  // Check for root AGENTS.md with OvseSpec markers
  ...
  if (await FileSystemUtils.fileExists(rootAgentsPath)) {
    ...
    hasRootAgentsWithMarkers = hasOvseSpecMarkers(content);
  }
...
/**
 * Checks if content contains OvseSpec markers.
 *
 * @param content - File content to check
 * @returns True if both start and end markers are present
 */
export function hasOvseSpecMarkers(content: string): boolean {
  return (
    content.includes(OVSESPEC_MARKERS.start) && content.includes(OVSESPEC_MARKERS.end)
  );
}

/**
 * Checks if file content is 100% OvseSpec content (only markers and whitespace outside).
 *
 * @param content - File content to check
 * @returns True if content outside markers is only whitespace
 */
export function isOnlyOvseSpecContent(content: string): boolean {
  const startIndex = content.indexOf(OVSESPEC_MARKERS.start);
  const endIndex = content.indexOf(OVSESPEC_MARKERS.end);
  ...
  const after = content.substring(endIndex + OVSESPEC_MARKERS.end.length);

  return before.trim() === '' && after.trim() === '';
}

/**
 * Removes the OvseSpec marker block from file content.
 ...
 *
 * @param content - File content with OvseSpec markers
 * @returns Content with marker block removed
 */
export function removeMarkerBlock(content: string): string {
  return removeMarkerBlockUtil(content, OVSESPEC_MARKERS.start, OVSESPEC_MARKERS.end);
}
...
/**
 * Cleans up legacy OvseSpec artifacts from a project.
 * Preserves ovsespec/project.md (shows migration hint instead of deleting).
 *
 * @param projectPath - The root path of the project
...
export async function cleanupLegacyArtifacts(
  ...
  // Delete legacy slash command directories (these are 100% OvseSpec-managed)
  ...
  // Delete legacy slash command files (these are 100% OvseSpec-managed)
  ...
  // Delete ovsespec/AGENTS.md (this is inside ovsespec/, it's OvseSpec-managed)
  if (detection.hasOpenspecAgents) {
    const agentsPath = FileSystemUtils.joinPath(projectPath, 'ovsespec', 'AGENTS.md');
    if (await FileSystemUtils.fileExists(agentsPath)) {
      try {
        await fs.unlink(agentsPath);
        result.deletedFiles.push('ovsespec/AGENTS.md');
      } catch (error: any) {
        result.errors.push(`Failed to delete ovsespec/AGENTS.md: ${error.message}`);
      }
    }
  }

  // Handle root AGENTS.md with OvseSpec markers - remove markers only, NEVER delete
...
export function formatCleanupSummary(result: CleanupResult): string {
  ...
  if (result.deletedFiles.length > 0 || result.deletedDirs.length > 0 || result.modifiedFiles.length > 0) {
    ...
    for (const dir of result.deletedDirs) {
      lines.push(`  ✓ Removed ${dir}/ (replaced by /ovsx:*)`);
    }

    for (const file of result.modifiedFiles) {
      lines.push(`  ✓ Removed OvseSpec markers from ${file}`);
    }
  }
 ...
 * Only includes OvseSpec-managed files (slash commands, ovsespec/AGENTS.md).
...
function buildRemovalsList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  // Slash command directories (these are 100% OvseSpec-managed)
  ...
  // Slash command files (these are 100% OvseSpec-managed)
  for (const file of detection.slashCommandFiles) {
    removals.push({ path: file, explanation: 'replaced by skills/' });
  }

  // ovsespec/AGENTS.md (inside ovsespec/, it's OvseSpec-managed)
  if (detection.hasOpenspecAgents) {
    removals.push({ path: 'ovsespec/AGENTS.md', explanation: 'obsolete workflow file' });
  }
...
function buildUpdatesList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  for (const file of detection.configFilesToUpdate) {
    updates.push({ path: file, explanation: 'removing OvseSpec markers' });
  }
...
export function formatDetectionSummary(detection: LegacyDetectionResult): string {
  ...
  lines.push(chalk.bold('Upgrading to the new OvseSpec'));
  lines.push('');
  lines.push('OvseSpec now uses agent skills, the emerging standard across coding');
  ...
  if (updates.length > 0) {
    ...
    lines.push(chalk.dim('OvseSpec markers will be removed, your content preserved:'));
  ...
  for (const file of detection.slashCommandFiles) {
    ...
    for (const [toolId, pattern] of Object.entries(LEGACY_SLASH_COMMAND_PATHS)) {
      if (pattern.type === 'files' && pattern.pattern) {
        ...
        // e.g., '.cursor/commands/ovsespec-*.md' -> /^\.cursor\/commands\/ovsespec-.*\.md$/
        const patterns = Array.isArray(pattern.pattern) ? pattern.pattern : [pattern.pattern];
        let matched = false;
        for (const p of patterns) {
...
export function formatProjectMdMigrationHint(): string {
  ...
  lines.push('  • ovsespec/project.md');
  lines.push(chalk.dim('    We won\'t delete this file. It may contain useful project context.'));
  lines.push('');
  lines.push(chalk.dim('    The new ovsespec/config.yaml has a "context:" section for planning'));
  lines.push(chalk.dim('    context. This is included in every OvseSpec request and works more'));
