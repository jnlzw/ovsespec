...
export class FishGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      topLevelLines.push(
        `complete -c ovsespec -n '__fish_ovsespec_no_subcommand' -a '${cmd.name}' -d '${this.escapeDescription(cmd.description)}'`
      );
    }
  ...
  private generateCommandCompletions(cmd: CommandDefinition): string[] {
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        lines.push(
          `complete -c ovsespec -n '__fish_ovsespec_using_subcommand ${cmd.name}; and not __fish_ovsespec_using_subcommand ${subcmd.name}' -a '${subcmd.name}' -d '${this.escapeDescription(subcmd.description)}'`
        );
      }
      lines.push('');
  ...
  private generateFlagCompletion(flag: FlagDefinition, condition: string): string[] {
    ...
    if (flag.takesValue && flag.values) {
      ...
      for (const value of flag.values) {
        if (shortFlag) {
          lines.push(
            `complete -c ovsespec -n '${condition}' -s ${flag.short} -l ${flag.name} -a '${value}' -d '${this.escapeDescription(flag.description)}'`
          );
        } else {
          lines.push(
            `complete -c ovsespec -n '${condition}' -l ${flag.name} -a '${value}' -d '${this.escapeDescription(flag.description)}'`
          );
        }
      }
      ...
      if (shortFlag) {
        lines.push(
          `complete -c ovsespec -n '${condition}' -s ${flag.short} -l ${flag.name} -r -d '${this.escapeDescription(flag.description)}'`
        );
      } else {
        lines.push(
          `complete -c ovsespec -n '${condition}' -l ${flag.name} -r -d '${this.escapeDescription(flag.description)}'`
        );
      }
    } else {
      // Boolean flag
      if (shortFlag) {
        lines.push(
          `complete -c ovsespec -n '${condition}' -s ${flag.short} -l ${flag.name} -d '${this.escapeDescription(flag.description)}'`
        );
      } else {
        lines.push(
          `complete -c ovsespec -n '${condition}' -l ${flag.name} -d '${this.escapeDescription(flag.description)}'`
        );
      }
    }
