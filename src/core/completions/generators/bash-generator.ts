...
/**
 * Generates Bash completion scripts for the OvseSpec CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for OvseSpec CLI
...
${helpers}
complete -F _ovsespec_completion ovsespec
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_ovsespec_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_ovsespec_complete_specs`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}_ovsespec_complete_items`);
        break;
      case 'schema-name':
        lines.push(`${indent}_ovsespec_complete_schemas`);
        break;
      case 'shell':
