...
import { OVSESPEC_DIR_NAME } from '../core/config.js';
...
const WORKFLOW_PROMPT_META: Record<string, WorkflowPromptMeta> = {
  ...
  onboard: {
    name: 'Onboard',
    description: 'Guided onboarding flow for OvseSpec',
  },
  'paas-test-deploy': {
...
function maybeWarnConfigDrift(
  ...
  const ovsespecDir = path.join(projectDir, OVSESPEC_DIR_NAME);
  if (!fs.existsSync(ovsespecDir)) {
    return;
  }
  if (!hasProjectConfigDrift(projectDir, state.workflows, state.delivery)) {
    return;
  }
  console.log(colorize('Warning: Global config is not applied to this project. Run `ovsespec update` to sync.'));
}
...
export function registerConfigCommand(program: Command): void {
  const configCmd = program
    .command('config')
    .description('View and modify global OvseSpec configuration')
  ...
  configCmd
    ...
    .action((key: string, value: string, options: { string?: boolean; allowUnknown?: boolean }) => {
      ...
      if (!keyValidation.valid && !allowUnknown) {
        ...
        console.error('Use "ovsespec config list" to see available keys.');
  ...
  configCmd
    ...
    .action(async (options: { all?: boolean; yes?: boolean }) => {
      if (!options.all) {
        ...
        console.error('Usage: ovsespec config reset --all [-y]');
        process.exitCode = 1;
        return;
      }
  ...
  configCmd
    ...
    .action(async (preset?: string) => {
      // Preset shortcut: `ovsespec config profile core`
      if (preset === 'core') {
        const config = getGlobalConfig();
        config.profile = 'core';
        ...
        console.log('Config updated. Run `ovsespec update` in your projects to apply.');
        return;
      }
      ...
      if (!process.stdout.isTTY) {
        console.error('Interactive mode required. Use `ovsespec config profile core` or set config via environment/flags.');
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        // Check if inside an OvseSpec project
        ...
        if (fs.existsSync(ovsespecDir)) {
          ...
          if (applyNow) {
            try {
              execSync('npx ovsespec update', { stdio: 'inherit', cwd: projectDir });
              console.log('Run `ovsespec update` in your other projects to apply.');
            } catch {
              console.error('`ovsespec update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }
