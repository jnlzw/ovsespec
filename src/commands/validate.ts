...
export class ValidateCommand {
  ...
  private printNonInteractiveHint(): void {
    ...
    console.error('  ovsespec validate --all');
    console.error('  ovsespec validate --changes');
    console.error('  ovsespec validate --specs');
    console.error('  ovsespec validate <item-name>');
  ...
  private async validateDirectItem(itemName: string, opts: { typeOverride?: ItemType; strict: boolean; json: boolean }): Promise<void> {
    ...
    if (!opts.typeOverride && isChange && isSpec) {
      ...
      console.error('Pass --type change|spec, or use: ovsespec change validate / ovsespec spec validate');
      process.exitCode = 1;
      return;
    }
  ...
  private async validateByType(type: ItemType, id: string, opts: { strict: boolean; json: boolean }): Promise<void> {
    ...
    if (type === 'change') {
      const changeDir = path.join(process.cwd(), 'ovsespec', 'changes', id);
    ...
    const file = path.join(process.cwd(), 'ovsespec', 'specs', id, 'spec.md');
    const start = Date.now();
  ...
  private printNextSteps(type: ItemType): void {
    ...
    if (type === 'change') {
      ...
      bullets.push('- Debug parsed deltas: ovsespec change show <id> --json --deltas-only');
    } else {
  ...
  private async runBulkValidation(scope: { changes: boolean; specs: boolean }, opts: { strict: boolean; json: boolean; concurrency?: string; noInteractive?: boolean }): Promise<void> {
    ...
    const concurrency = normalizeConcurrency(opts.concurrency) ?? normalizeConcurrency(process.env.OVSESPEC_CONCURRENCY) ?? DEFAULT_CONCURRENCY;
    const validator = new Validator(opts.strict);
