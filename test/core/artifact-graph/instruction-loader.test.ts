...
describe('instruction-loader', () => {
  ...
  describe('loadChangeContext', () => {
    ...
    beforeEach(() => {
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ovsespec-test-'));
    });
    ...
    it('should detect completed artifacts', () => {
      ...
      const changeDir = path.join(tempDir, 'ovsespec', 'changes', 'my-change');
    ...
    it('should auto-detect schema from .ovsespec.yaml metadata', () => {
      // Create change directory with metadata file
      const changeDir = path.join(tempDir, 'ovsespec', 'changes', 'my-change');
      fs.mkdirSync(changeDir, { recursive: true });
      fs.writeFileSync(path.join(changeDir, '.ovsespec.yaml'), 'schema: spec-driven\ncreated: "2025-01-05"\n');
...
`
      ...
      it('should deduplicate validation warnings within session', () => {
        ...
        const freshTempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ovsespec-test-'));

        try {
          // Create project config with a uniquely named invalid artifact ID
          const configDir = path.join(freshTempDir, 'ovsespec');
          fs.mkdirSync(configDir, { recursive: true });
          fs.writeFileSync(
            path.join(configDir, 'config.yaml'),
            `schema: spec-driven
