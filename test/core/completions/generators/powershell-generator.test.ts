...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for OvseSpec CLI');
			expect(script).toContain('$ovsespecCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for ovsespec command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize OvseSpec',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName ovsespec');
			expect(script).toContain('-ScriptBlock $ovsespecCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-OvseSpecChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-OvseSpecSpecs');
		});
		...
		it('should handle positional arguments for schema names', () => {
			...
			expect(script).toContain('Get-OvseSpecSchemas');
			expect(script).toContain('ovsespec __complete schemas 2>$null');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-OvseSpecChanges');
			expect(script).toContain('ovsespec __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-OvseSpecSpecs');
			expect(script).toContain('ovsespec __complete specs 2>$null');
		});
