...
describe('ListCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `ovsespec-list-test-${Date.now()}`);
  ...
  describe('execute', () => {
    it('should handle missing ovsespec/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No OvseSpec changes directory found. Run 'ovsespec init' first."
      );
    });

    it('should handle empty changes directory', async () => {
      const changesDir = path.join(tempDir, 'ovsespec', 'changes');
