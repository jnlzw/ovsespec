...
describe('InitCommand', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `ovsespec-init-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid reading real config
    configTempDir = path.join(os.tmpdir(), `ovsespec-config-init-${Date.now()}`);
  ...
  describe('execute with --tools flag', () => {
    it('should create OvseSpec directory structure', async () => {
      const initCommand = new InitCommand({ tools: 'claude', force: true });

      await initCommand.execute(testDir);

      const ovsespecPath = path.join(testDir, 'ovsespec');
      expect(await directoryExists(ovsespecPath)).toBe(true);
      expect(await directoryExists(path.join(ovsespecPath, 'specs'))).toBe(true);
      expect(await directoryExists(path.join(ovsespecPath, 'changes'))).toBe(true);
      expect(await directoryExists(path.join(ovsespecPath, 'changes', 'archive'))).toBe(true);
    });
    ...
    it('should create config.yaml with default schema', async () => {
      ...
      const configPath = path.join(testDir, 'ovsespec', 'config.yaml');
    ...
    it('should not create prompt commands for Claude Code by default', async () => {
      ...
      const nonCoreCommandNames = [
        ...
        'ovsx/continue.md',
        'ovsx/ff.md',
        'ovsx/bulk-archive.md',
        'ovsx/verify.md',
        'ovsx/paas-test-deploy.md',
      ];
    ...
    it('should create skills in Cursor skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.cursor', 'skills', 'ovsespec-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should create skills in Windsurf skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.windsurf', 'skills', 'ovsespec-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should support Kimi CLI as an adapterless skills-only tool', async () => {
      ...
      const skillFile = path.join(testDir, '.kimi', 'skills', 'ovsespec-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    ...
    it('should create skills for multiple tools at once', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'ovsespec-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'ovsespec-explore', 'SKILL.md');
    ...
    it('should select all tools with --tools all option', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'ovsespec-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'ovsespec-explore', 'SKILL.md');
      const windsurfSkill = path.join(testDir, '.windsurf', 'skills', 'ovsespec-explore', 'SKILL.md');
    ...
    it('should skip tool configuration with --tools none option', async () => {
      ...
      // Should create OvseSpec structure but no skills
      const ovsespecPath = path.join(testDir, 'ovsespec');
      expect(await directoryExists(ovsespecPath)).toBe(true);
    ...
    it('should not create config.yaml if it already exists', async () => {
      ...
      const configPath = path.join(ovsespecDir, 'config.yaml');
    ...
    it('should handle non-existent target directory', async () => {
      ...
      const ovsespecPath = path.join(newDir, 'ovsespec');
      expect(await directoryExists(ovsespecPath)).toBe(true);
    });
  ...
  describe('skill content validation', () => {
    it('should generate valid SKILL.md with YAML frontmatter', async () => {
      ...
      expect(content).toContain('name: ovsespec-explore');
      expect(content).toContain('description:');
    ...
    it('should include propose skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'ovsespec-propose', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: ovsespec-propose');
    });
    ...
    it('should include apply-change skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'ovsespec-apply-change', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: ovsespec-apply-change');
    });
  ...
  describe('command generation', () => {
    ...
    it('should generate Claude Code commands with correct format', async () => {
      ...
      const cmdFile = path.join(testDir, '.claude', 'commands', 'ovsx', 'explore.md');
    ...
    it('should generate Cursor commands with correct format', async () => {
      ...
      const cmdFile = path.join(testDir, '.cursor', 'commands', 'ovsx-explore.md');
      ...
      vi.spyOn(fs, 'writeFile').mockImplementation(
        async (filePath: any, ...args: any[]) => {
          if (
            ...
            filePath.includes('.ovsespec-test-')
          ) {
  ...
  describe('tool-specific adapters', () => {
    ...
    it('should generate Gemini CLI commands as TOML files', async () => {
      ...
      const cmdFile = path.join(testDir, '.gemini', 'commands', 'ovsx', 'explore.toml');
      expect(await fileExists(cmdFile)).toBe(true);
    ...
    it('should generate Windsurf commands', async () => {
      ...
      const cmdFile = path.join(testDir, '.windsurf', 'workflows', 'ovsx-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
    ...
    it('should generate Continue prompt files', async () => {
      ...
      const cmdFile = path.join(testDir, '.continue', 'prompts', 'ovsx-explore.prompt');
      expect(await fileExists(cmdFile)).toBe(true);
    ...
    it('should generate Cline workflow files', async () => {
      ...
      const cmdFile = path.join(testDir, '.clinerules', 'workflows', 'ovsx-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
    ...
    it('should generate GitHub Copilot prompt files', async () => {
      ...
      const cmdFile = path.join(testDir, '.github', 'prompts', 'ovsx-explore.prompt.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
  });
});
...
describe('InitCommand - profile and detection features', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `ovsespec-init-profile-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid polluting real config
    configTempDir = path.join(os.tmpdir(), `ovsespec-config-test-${Date.now()}`);
    await fs.mkdir(configTempDir, { recursive: true });
    process.env.XDG_CONFIG_HOME = configTempDir;
    vi.spyOn(console, 'log').mockImplementation(() => {});
    confirmMock.mockReset();
  ...
  it('should use --profile flag to override global config', async () => {
    ...
    const proposeSkill = path.join(testDir, '.claude', 'skills', 'ovsespec-propose', 'SKILL.md');
    expect(await fileExists(proposeSkill)).toBe(true);

    // Non-core skills (from the custom profile) should NOT be created
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'ovsespec-new-change', 'SKILL.md');
  ...
  it('should auto-cleanup legacy artifacts in non-interactive mode without --force', async () => {
    ...
    await fs.writeFile(path.join(legacyDir, 'ovsx-propose.md'), 'legacy content');
  ...
  it('should preselect configured tools but not directory-detected tools in extend mode', async () => {
    // Simulate existing OvseSpec project (extend mode).
    await fs.mkdir(path.join(testDir, 'ovsespec'), { recursive: true });

    // Configured with OvseSpec
    ...
    // Directory detected only (not configured with OvseSpec)
  ...
  it('should preselect detected tools for first-time interactive setup', async () => {
    // First-time init: no ovsespec/ directory and no configured OvseSpec skills.
    await fs.mkdir(path.join(testDir, '.github'), { recursive: true });
    await fs.writeFile(path.join(testDir, '.github', 'copilot-instructions.md'), '');
  ...
  it('should respect custom profile from global config', async () => {
    ...
    const exploreSkill = path.join(testDir, '.claude', 'skills', 'ovsespec-explore', 'SKILL.md');
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'ovsespec-new-change', 'SKILL.md');
  ...
  it('should migrate commands-only extend mode to custom profile without injecting propose', async () => {
    await fs.mkdir(path.join(testDir, 'ovsespec'), { recursive: true });
    await fs.mkdir(path.join(testDir, '.claude', 'commands', 'ovsx'), { recursive: true });
    await fs.writeFile(path.join(testDir, '.claude', 'commands', 'ovsx', 'explore.md'), '# explore\n');
    ...
    const exploreCommand = path.join(testDir, '.claude', 'commands', 'ovsx', 'explore.md');
    const proposeCommand = path.join(testDir, '.claude', 'commands', 'ovsx', 'propose.md');
