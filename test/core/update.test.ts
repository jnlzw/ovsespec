...
describe('UpdateCommand', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `ovsespec-test-${randomUUID()}`);
    await fs.mkdir(testDir, { recursive: true });

    // Create ovsespec directory
    const ovsespecDir = path.join(testDir, 'ovsespec');
    await fs.mkdir(ovsespecDir, { recursive: true });
  ...
  describe('basic validation', () => {
    it('should throw error if ovsespec directory does not exist', async () => {
      // Remove ovsespec directory
      await fs.rm(path.join(testDir, 'ovsespec'), {
        recursive: true,
        force: true,
      });

      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "No OvseSpec directory found. Run 'ovsespec init' first."
      );
    });
...
`;
      ...
      expect(updatedSkill).toContain('name: ovsespec-explore');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'ovsespec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'ovsespec-explore', 'SKILL.md'),
        'old content'
      );
  ...
  describe('command updates', () => {
    it('should update ovsx commands for configured Claude tool', async () => {
      ...
      // Check ovsx command files were created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'ovsx');
    ...
    it('should update core profile ovsx commands when tool is configured', async () => {
      ...
      const nonCoreCommandIds = ['new', 'continue', 'ff', 'bulk-archive', 'verify', 'paas-test-deploy'];
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'ovsespec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'ovsespec-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'ovsespec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'ovsespec-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: ovsespec-explore');
      ...
      expect(cursorSkill).toContain('name: ovsespec-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'ovsespec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'ovsespec-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (TOML) - Qwen uses flat path structure: ovsx-<id>.toml
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'ovsespec-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'ovsespec-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'ovsespec-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'ovsespec-apply-change', 'SKILL.md'),
        'old'
      );
...
`
    ...
    it('should update all tools when force is used with mixed versions', async () => {
      ...
      const { version } = await import('../../package.json');
      const claudeSkillDir = path.join(testDir, '.claude', 'skills', 'ovsespec-explore');
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'ovsespec-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'ovsespec-explore', 'SKILL.md');
  ...
  describe('legacy cleanup', () => {
    it('should detect and auto-cleanup legacy files with --force flag', async () => {
      ...
      // Create legacy CLAUDE.md with OvseSpec markers
      const legacyContent = `${OVSESPEC_MARKERS.start}
...
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Upgrading to the new OvseSpec')
      );

      // Should show marker removal message (config files are never deleted, only have markers removed)
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed OvseSpec markers from CLAUDE.md')
      );
...
`;
    ...
    it('should cleanup legacy slash command directories with --force', async () => {
      ...
      const legacyCommandDir = path.join(testDir, '.claude', 'commands', 'ovsespec');
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed .claude/commands/ovsespec/')
      );
    ...
    it('should cleanup legacy ovsespec/AGENTS.md with --force', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'ovsespec-explore'), {
        recursive: true,
      ...
      // Create legacy ovsespec/AGENTS.md
      await fs.writeFile(
        path.join(testDir, 'ovsespec', 'AGENTS.md'),
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed ovsespec/AGENTS.md')
      );
    ...
    it('should not show legacy cleanup messages when no legacy files exist', async () => {
      ...
      const hasLegacyMessage = calls.some(call =>
        call.includes('Upgrading to the new OvseSpec')
      );
    ...
    it('should remove OvseSpec marker block from mixed content files', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'ovsespec-explore'), {
        recursive: true,
      ...
      // Create CLAUDE.md with mixed content (user content + OvseSpec markers)
...
`;
      ...
      expect(updatedContent).not.toContain(OVSESPEC_MARKERS.start);
      expect(updatedContent).not.toContain(OVSESPEC_MARKERS.end);
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('/ovsx:new')
      );

      // Skills should be created
      const skillFile = path.join(testDir, '.claude', 'skills', 'ovsespec-explore', 'SKILL.md');
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, '.claude', 'commands', 'ovsespec', 'proposal.md'),
        'content'
      );

      await fs.mkdir(path.join(testDir, '.cursor', 'commands'), { recursive: true });
      await fs.writeFile(
        path.join(testDir, '.cursor', 'commands', 'ovsespec-proposal.md'),
        'content'
      );
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'ovsespec-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'ovsespec-explore', 'SKILL.md');
    ...
    it('should not upgrade legacy tools already configured', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'ovsespec-explore'), { recursive: true });
      await fs.writeFile(
        path.join(skillsDir, 'ovsespec-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'ovsespec-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'ovsespec-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'ovsespec-new-change', 'SKILL.md');
    ...
    it('should create commands when upgrading legacy tools', async () => {
      ...
      // New ovsx commands should be created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'ovsx');
      const exploreCmd = path.join(commandsDir, 'explore.md');
      const exists = await FileSystemUtils.fileExists(exploreCmd);
      expect(exists).toBe(true);
    ...
    it('should not inject non-profile workflows when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'ovsespec-propose', 'SKILL.md')
      )).toBe(false);
  ...
  describe('profile-aware updates', () => {
    it('should generate only profile workflows when custom profile is set', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'ovsespec-explore'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'ovsespec-explore', 'SKILL.md'), 'old');
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'ovsespec-new-change', 'SKILL.md')
      )).toBe(true);
    ...
    it('should suggest core preset when custom profile preserves the old core workflow set', async () => {
      ...
      expect(calls.some(call =>
        call.includes('ovsespec config profile core') && call.includes('ovsespec update')
      )).toBe(true);

      expect(await FileSystemUtils.fileExists(
        path.join(testDir, '.claude', 'skills', 'ovsespec-sync-specs', 'SKILL.md')
      )).toBe(false);
      expect(await FileSystemUtils.fileExists(
        path.join(testDir, '.claude', 'commands', 'ovsx', 'sync.md')
      )).toBe(false);
...
`
    ...
    it('should remove workflows outside profile during update sync', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'ovsespec-new-change'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'ovsespec-new-change', 'SKILL.md'), 'old');
      const extraCommandFile = path.join(testDir, '.claude', 'commands', 'ovsx', 'new.md');
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'ovsespec-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'ovsespec-explore', 'SKILL.md'), 'old');
      ...
      const hasNewToolMessage = calls.some(call =>
        call.includes("Detected new tool: Cursor. Run 'ovsespec init' to add it.")
      );
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("Run 'ovsespec init' to add them.");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'ovsespec-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'ovsespec-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'ovsespec-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'ovsespec-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'ovsespec-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'ovsespec-apply-change', 'SKILL.md'), 'content');
