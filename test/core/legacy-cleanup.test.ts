...
import { OVSESPEC_MARKERS } from '../../src/core/config.js';
...
describe('legacy-cleanup', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `ovsespec-legacy-test-${randomUUID()}`);
    await fs.mkdir(testDir, { recursive: true });
    // Create ovsespec directory structure
    await fs.mkdir(path.join(testDir, 'ovsespec'), { recursive: true });
  });
  ...
  describe('hasOvseSpecMarkers', () => {
    it('should return true when both markers are present', () => {
      const content = `Some content
${OVSESPEC_MARKERS.start}
OvseSpec content
${OVSESPEC_MARKERS.end}
More content`;
      expect(hasOvseSpecMarkers(content)).toBe(true);
    });
...
OvseSpec content
${OVSESPEC_MARKERS.end}`;
      expect(hasOvseSpecMarkers(content)).toBe(false);
    });

    it('should return false when end marker is missing', () => {
      const content = `${OVSESPEC_MARKERS.start}
OvseSpec content
Some content`;
      expect(hasOvseSpecMarkers(content)).toBe(false);
    });
  ...
  describe('isOnlyOvseSpecContent', () => {
    it('should return true when content is only markers and whitespace outside', () => {
      const content = `${OVSESPEC_MARKERS.start}
OvseSpec content here
${OVSESPEC_MARKERS.end}`;
      expect(isOnlyOvseSpecContent(content)).toBe(true);
    });
...
OvseSpec content
${OVSESPEC_MARKERS.end}`;
      expect(isOnlyOvseSpecContent(content)).toBe(false);
    });
...
User content here`;
    ...
    it('should return false when end marker comes before start marker', () => {
      const content = `${OVSESPEC_MARKERS.end}
Content
${OVSESPEC_MARKERS.start}`;
      expect(isOnlyOvseSpecContent(content)).toBe(false);
    });
...
${OVSESPEC_MARKERS.end}`;
      ...
      expect(result).not.toContain(OVSESPEC_MARKERS.start);
      expect(result).not.toContain(OVSESPEC_MARKERS.end);
    });
...
${OVSESPEC_MARKERS.start}`;
      const result = removeMarkerBlock(content);
      expect(result).toContain(OVSESPEC_MARKERS.end);
      expect(result).toContain(OVSESPEC_MARKERS.start);
    });

    it('should ignore inline mentions of markers and only remove actual block', () => {
      const content = `Intro referencing ${OVSESPEC_MARKERS.start} and ${OVSESPEC_MARKERS.end} inline.
...
After content`;
  ...
  describe('detectLegacyConfigFiles', () => {
    it('should detect CLAUDE.md with OvseSpec markers and put in update list', async () => {
      const claudePath = path.join(testDir, 'CLAUDE.md');
      await fs.writeFile(claudePath, `${OVSESPEC_MARKERS.start}
OvseSpec content
${OVSESPEC_MARKERS.end}`);
...
${OVSESPEC_MARKERS.end}`);
    ...
    it('should not detect files without OvseSpec markers', async () => {
      const claudePath = path.join(testDir, 'CLAUDE.md');
    ...
    it('should detect multiple config files', async () => {
      ...
      await fs.writeFile(path.join(testDir, 'CLAUDE.md'), `${OVSESPEC_MARKERS.start}\nContent\n${OVSESPEC_MARKERS.end}`);
      await fs.writeFile(path.join(testDir, 'CLINE.md'), `${OVSESPEC_MARKERS.start}\nContent\n${OVSESPEC_MARKERS.end}`);
      await fs.writeFile(path.join(testDir, 'QODER.md'), `${OVSESPEC_MARKERS.start}\nContent\n${OVSESPEC_MARKERS.end}`);
  ...
  describe('detectLegacySlashCommands', () => {
    it('should detect legacy Claude slash command directory', async () => {
      const dirPath = path.join(testDir, '.claude', 'commands', 'ovsespec');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'proposal.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.directories).toContain('.claude/commands/ovsespec');
    });

    it('should detect legacy Cursor slash command files', async () => {
      const dirPath = path.join(testDir, '.cursor', 'commands');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'ovsespec-proposal.md'), 'content');
      await fs.writeFile(path.join(dirPath, 'ovsespec-apply.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.cursor/commands/ovsespec-proposal.md');
      expect(result.files).toContain('.cursor/commands/ovsespec-apply.md');
    });

    it('should detect legacy Windsurf workflow files', async () => {
      const dirPath = path.join(testDir, '.windsurf', 'workflows');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'ovsespec-archive.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.windsurf/workflows/ovsespec-archive.md');
    });

    it('should detect multiple tool directories and files', async () => {
      // Create directory-based
      await fs.mkdir(path.join(testDir, '.claude', 'commands', 'ovsespec'), { recursive: true });
      await fs.mkdir(path.join(testDir, '.qoder', 'commands', 'ovsespec'), { recursive: true });

      // Create file-based
      await fs.mkdir(path.join(testDir, '.cursor', 'commands'), { recursive: true });
      await fs.writeFile(path.join(testDir, '.cursor', 'commands', 'ovsespec-proposal.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.directories).toContain('.claude/commands/ovsespec');
      expect(result.directories).toContain('.qoder/commands/ovsespec');
      expect(result.files).toContain('.cursor/commands/ovsespec-proposal.md');
    });

    it('should not detect non-ovsespec files', async () => {
      const dirPath = path.join(testDir, '.cursor', 'commands');
      await fs.mkdir(dirPath, { recursive: true });
    ...
    it('should detect TOML-based slash commands for Qwen', async () => {
      ...
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'ovsespec-proposal.toml'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.qwen/commands/ovsespec-proposal.toml');
    });

    it('should detect Continue prompt files', async () => {
      const dirPath = path.join(testDir, '.continue', 'prompts');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'ovsespec-apply.prompt'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.continue/prompts/ovsespec-apply.prompt');
    });

    it('should detect legacy OpenCode ovsx-* command files', async () => {
      const dirPath = path.join(testDir, '.opencode', 'command');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'ovsx-propose.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.opencode/command/ovsx-propose.md');
    });

    it('should detect legacy OpenCode ovsespec-* command files', async () => {
      const dirPath = path.join(testDir, '.opencode', 'command');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'ovsespec-new.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.opencode/command/ovsespec-new.md');
    });

    it('should detect both ovsx-* and ovsespec-* OpenCode command files', async () => {
      const dirPath = path.join(testDir, '.opencode', 'command');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'ovsx-propose.md'), 'content');
      await fs.writeFile(path.join(dirPath, 'ovsespec-new.md'), 'content');
  ...
  describe('detectLegacyStructureFiles', () => {
    it('should detect ovsespec/AGENTS.md', async () => {
      const agentsPath = path.join(testDir, 'ovsespec', 'AGENTS.md');
    ...
    it('should detect ovsespec/project.md', async () => {
      const projectPath = path.join(testDir, 'ovsespec', 'project.md');
    ...
    it('should detect root AGENTS.md with OvseSpec markers', async () => {
      const agentsPath = path.join(testDir, 'AGENTS.md');
      await fs.writeFile(agentsPath, `${OVSESPEC_MARKERS.start}
OvseSpec content
${OVSESPEC_MARKERS.end}`);
  ...
  describe('detectLegacyArtifacts', () => {
    ...
    it('should return hasLegacyArtifacts: true when slash commands are found', async () => {
      ...
      const result = await detectLegacyArtifacts(testDir);
      expect(result.hasLegacyArtifacts).toBe(true);
      expect(result.slashCommandDirs).toContain('.claude/commands/ovsespec');
    });

    it('should return hasLegacyArtifacts: true when ovsespec/AGENTS.md is found', async () => {
      await fs.writeFile(path.join(testDir, 'ovsespec', 'AGENTS.md'), 'content');
    ...
    it('should detect project.md for migration hint (it is preserved, not deleted)', async () => {
      await fs.writeFile(path.join(testDir, 'ovsespec', 'project.md'), 'content');
  ...
  describe('cleanupLegacyArtifacts', () => {
    it('should remove markers from config files that have only OvseSpec content (never delete)', async () => {
      const claudePath = path.join(testDir, 'CLAUDE.md');
      await fs.writeFile(claudePath, `${OVSESPEC_MARKERS.start}\nContent\n${OVSESPEC_MARKERS.end}`);
      ...
      expect(content).not.toContain(OVSESPEC_MARKERS.start);
      expect(content).not.toContain(OVSESPEC_MARKERS.end);
    });
...
${OVSESPEC_MARKERS.end}`);
    ...
    it('should delete legacy slash command directories', async () => {
      ...
      expect(result.deletedDirs).toContain('.claude/commands/ovsespec');
    ...
    it('should delete legacy slash command files', async () => {
      const dirPath = path.join(testDir, '.cursor', 'commands');
      await fs.mkdir(dirPath, { recursive: true });
      const filePath = path.join(dirPath, 'ovsespec-proposal.md');
      await fs.writeFile(filePath, 'content');

      const detection = await detectLegacyArtifacts(testDir);
      const result = await cleanupLegacyArtifacts(testDir, detection);

      expect(result.deletedFiles).toContain('.cursor/commands/ovsespec-proposal.md');
      await expect(fs.access(filePath)).rejects.toThrow();
    });

    it('should delete ovsespec/AGENTS.md', async () => {
      const agentsPath = path.join(testDir, 'ovsespec', 'AGENTS.md');
      ...
      expect(result.deletedFiles).toContain('ovsespec/AGENTS.md');
      await expect(fs.access(agentsPath)).rejects.toThrow();
      // ovsespec directory should still exist
      await expect(fs.access(path.join(testDir, 'ovsespec'))).resolves.not.toThrow();
    });

    it('should NOT delete ovsespec/project.md', async () => {
      const projectPath = path.join(testDir, 'ovsespec', 'project.md');
      ...
      expect(result.deletedFiles).not.toContain('ovsespec/project.md');
...
${OVSESPEC_MARKERS.end}`);
    ...
    it('should remove markers from root AGENTS.md even when only OvseSpec content (never delete)', async () => {
      const agentsPath = path.join(testDir, 'AGENTS.md');
      await fs.writeFile(agentsPath, `${OVSESPEC_MARKERS.start}\nOvseSpec content\n${OVSESPEC_MARKERS.end}`);
  ...
  describe('formatCleanupSummary', () => {
    ...
    it('should format deleted directories', () => {
      const result = {
        deletedFiles: [],
        modifiedFiles: [],
        deletedDirs: ['.claude/commands/ovsespec'],
        projectMdNeedsMigration: false,
        errors: [],
      };

      const summary = formatCleanupSummary(result);
      expect(summary).toContain('✓ Removed .claude/commands/ovsespec/ (replaced by /ovsx:*)');
    });
    ...
    it('should format modified files', () => {
      ...
      const summary = formatCleanupSummary(result);
      expect(summary).toContain('✓ Removed OvseSpec markers from AGENTS.md');
    });
    ...
    it('should include migration hint for project.md', () => {
      ...
      expect(summary).toContain('ovsespec/project.md');
  ...
  describe('formatDetectionSummary', () => {
    it('should include welcoming upgrade header and explanation', () => {
      ...
      expect(summary).toContain('Upgrading to the new OvseSpec');
    ...
    it('should format slash command directories', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: ['.claude/commands/ovsespec'],
        slashCommandFiles: [],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
      ...
      expect(summary).toContain('• .claude/commands/ovsespec/');
    });
    ...
    it('should format slash command files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.cursor/commands/ovsespec-proposal.md'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
      ...
      const summary = formatDetectionSummary(detection);
      expect(summary).toContain('Files to remove');
      expect(summary).toContain('• .cursor/commands/ovsespec-proposal.md');
    });

    it('should format ovsespec/AGENTS.md', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
      ...
      const summary = formatDetectionSummary(detection);
      expect(summary).toContain('Files to remove');
      expect(summary).toContain('• ovsespec/AGENTS.md');
    });
    ...
    it('should group both removals and updates correctly', () => {
      ...
      expect(summary).toContain('Files to remove');
      expect(summary).toContain('Files to update');
      // Check removals (only slash commands and ovsespec/AGENTS.md)
      expect(summary).toContain('• .claude/commands/ovsespec/');
      expect(summary).toContain('• ovsespec/AGENTS.md');
  ...
  describe('formatProjectMdMigrationHint', () => {
    it('should return migration hint message', () => {
      ...
      expect(hint).toContain('ovsespec/project.md');
    ...
    it('should explain the new context section benefits', () => {
      const hint = formatProjectMdMigrationHint();
      expect(hint).toContain('included in every OvseSpec request');
  ...
  describe('LEGACY_SLASH_COMMAND_PATHS', () => {
    it('should include expected tool patterns', () => {
      expect(LEGACY_SLASH_COMMAND_PATHS['claude']).toEqual({
        type: 'directory',
        path: '.claude/commands/ovsespec',
      });

      expect(LEGACY_SLASH_COMMAND_PATHS['cursor']).toEqual({
        type: 'files',
        pattern: '.cursor/commands/ovsespec-*.md',
      });

      expect(LEGACY_SLASH_COMMAND_PATHS['windsurf']).toEqual({
        type: 'files',
        pattern: '.windsurf/workflows/ovsespec-*.md',
      });
    });
  ...
  describe('getToolsFromLegacyArtifacts', () => {
    ...
    it('should extract multiple tools from mixed legacy artifacts', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: ['.claude/commands/ovsespec', '.qoder/commands/ovsespec'],
        slashCommandFiles: ['.cursor/commands/ovsespec-apply.md', '.windsurf/workflows/ovsespec-archive.md'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should deduplicate tools when multiple files match same tool', () => {
      const detection = {
        ...
        slashCommandDirs: [],
        slashCommandFiles: [
          '.cursor/commands/ovsespec-proposal.md',
          '.cursor/commands/ovsespec-apply.md',
          '.cursor/commands/ovsespec-archive.md',
        ],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
    ...
    it('should handle qwen TOML-based legacy files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.qwen/commands/ovsespec-proposal.toml'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle continue prompt files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.continue/prompts/ovsespec-apply.prompt'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle github-copilot prompt files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.github/prompts/ovsespec-apply.prompt.md'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle opencode ovsx-* legacy files', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.opencode/command/ovsx-propose.md'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle opencode ovsespec-* legacy files', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.opencode/command/ovsespec-new.md'],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should deduplicate opencode when both ovsx-* and ovsespec-* files exist', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [
          '.opencode/command/ovsx-propose.md',
          '.opencode/command/ovsespec-new.md',
        ],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
