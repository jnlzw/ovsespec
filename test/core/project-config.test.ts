...
describe('project-config', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ovsespec-test-config-'));
...
`
      ...
      it('should handle completely invalid YAML gracefully', () => {
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(
          expect.stringContaining('Failed to parse ovsespec/config.yaml'),
          expect.anything()
        );
      });
...
`
    ...
    describe('.yml/.yaml precedence', () => {
      ...
      it('should return null when ovsespec directory does not exist', () => {
        const config = readProjectConfig(tempDir);
  ...
  describe('suggestSchemas', () => {
    ...
    it('should include fix instruction', () => {
      ...
      expect(message).toContain(
        "Fix: Edit ovsespec/config.yaml and change 'schema: wrong-schema' to a valid schema name"
      );
    });
