...
describe('migration', () => {
  ...
  beforeEach(async () => {
    projectDir = path.join(os.tmpdir(), `ovsespec-migration-project-${randomUUID()}`);
    configHome = path.join(os.tmpdir(), `ovsespec-migration-config-${randomUUID()}`);
  ...
  it('migrates to custom skills delivery when only managed skills are detected', async () => {
    await writeSkill(projectDir, 'ovsespec-explore');
    await writeSkill(projectDir, 'ovsespec-apply-change');
  ...
  it('ignores unknown custom skill and command files when scanning workflows', async () => {
    ...
    const customCommandPath = path.join(projectDir, '.claude', 'commands', 'ovsx', 'my-custom.md');
