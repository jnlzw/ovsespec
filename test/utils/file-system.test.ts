  ...
  describe('joinPath', () => {
    it('should join POSIX-style paths', () => {
      const result = FileSystemUtils.joinPath(
        '/tmp/project',
        '.claude/commands/ovsespec/proposal.md'
      );
      expect(result).toBe('/tmp/project/.claude/commands/ovsespec/proposal.md');
    });

    it('should join Linux home directory paths', () => {
      const result = FileSystemUtils.joinPath(
        '/home/dev/workspace/ovsespec',
        '.cursor/commands/install.md'
      );
      expect(result).toBe('/home/dev/workspace/ovsespec/.cursor/commands/install.md');
    });
    ...
    it('should join Windows drive-letter paths with backslashes', () => {
      ...
      expect(result).toBe(
        'C:\\Users\\dev\\project\\.claude\\commands\\ovsespec\\proposal.md'
      );
    });
    ...
    it('should join Windows paths that use forward slashes', () => {
      ...
      expect(result).toBe(
        'D:\\workspace\\app\\.cursor\\commands\\ovsespec-apply.md'
      );
    });
    ...
    it('should join UNC-style Windows paths', () => {
      const result = FileSystemUtils.joinPath(
        '\\server\\share\\repo',
        '.windsurf/workflows/ovsespec-archive.md'
      );
      expect(result).toBe(
        '\\server\\share\\repo\\.windsurf\\workflows\\ovsespec-archive.md'
      );
    });
  });
});
