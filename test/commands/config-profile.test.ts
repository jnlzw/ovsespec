...
async function runConfigCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'ovsespec', 'config', ...args]);
}
...
describe('config profile interactive flow', () => {
  ...
  function setupDriftedProjectArtifacts(projectDir: string): void {
    fs.mkdirSync(path.join(projectDir, 'ovsespec'), { recursive: true });
    const exploreSkillPath = path.join(projectDir, '.claude', 'skills', 'ovsespec-explore', 'SKILL.md');
    fs.mkdirSync(path.dirname(exploreSkillPath), { recursive: true });
    fs.writeFileSync(exploreSkillPath, 'name: ovsespec-explore\n', 'utf-8');
  }
  ...
  function setupSyncedCoreBothArtifacts(projectDir: string): void {
    ...
    for (const commandId of coreCommands) {
      const commandPath = path.join(projectDir, '.claude', 'commands', 'ovsx', `${commandId}.md`);
  ...
  function addExtraVerifyWorkflowArtifacts(projectDir: string): void {
    const verifySkillPath = path.join(projectDir, '.claude', 'skills', 'ovsespec-verify-change', 'SKILL.md');
    fs.mkdirSync(path.dirname(verifySkillPath), { recursive: true });
    fs.writeFileSync(verifySkillPath, 'name: ovsespec-verify-change\n', 'utf-8');

    const verifyCommandPath = path.join(projectDir, '.claude', 'commands', 'ovsx', 'verify.md');
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `ovsespec-config-profile-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('selecting current values only should be a no-op and should not ask apply', async () => {
    ...
    fs.mkdirSync(path.join(tempDir, 'ovsespec'), { recursive: true });
    select.mockResolvedValueOnce('delivery');
