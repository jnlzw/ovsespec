...
OvseSpec installation paths are currently inconsistent:
...
- Add install scope control in `ovsespec config profile` interactive flow.
- Extend `ovsespec config list` output with install scope source (`explicit`, `new-default`, `legacy-default`).
