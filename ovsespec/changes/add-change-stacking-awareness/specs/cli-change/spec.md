...
- **WHEN** a user runs `ovsespec change graph`
...
- **WHEN** a user runs `ovsespec change next`
...
- **WHEN** a user runs `ovsespec change split <change-id>`
...
- **WHEN** a user runs `ovsespec change split <change-id>` for a parent whose generated child directories already exist
