...
- **AND** file path SHALL follow pattern `.claude/commands/ovsx/<id>.md`

#### Scenario: Cursor adapter formatting

- **WHEN** formatting a command for Cursor
- **THEN** the adapter SHALL output YAML frontmatter with `name` as `/ovsx-<id>`, `id`, `category`, `description` fields
- **AND** file path SHALL follow pattern `.cursor/commands/ovsx-<id>.md`

#### Scenario: Windsurf adapter formatting

- **WHEN** formatting a command for Windsurf
- **THEN** the adapter SHALL output YAML frontmatter with `name`, `description`, `category`, `tags` fields
- **AND** file path SHALL follow pattern `.windsurf/workflows/ovsx-<id>.md`
...
- **AND** file path SHALL follow pattern `.opencode/commands/ovsx-<id>.md` using `path.join('.opencode', 'commands', ...)` for cross-platform compatibility
- **AND** the adapter SHALL transform colon-based command references (`/ovsx:name`) to hyphen-based (`/ovsx-name`) in the body
...
- **WHEN** running legacy artifact detection on a project with files matching `.opencode/command/ovsx-*.md` or `.opencode/command/ovsespec-*.md`
...
- **WHEN** a user runs `ovsespec init` in a project with old `.opencode/command/` artifacts
...
- **WHEN** a user runs `ovsespec init` in non-interactive mode (e.g., CI) and legacy artifacts are detected
- **THEN** the system SHALL auto-cleanup legacy artifacts without requiring `--force`
- **AND** legacy slash command files (100% OvseSpec-managed) SHALL be removed
- **AND** config file cleanup SHALL only remove OvseSpec markers (never delete user files)
