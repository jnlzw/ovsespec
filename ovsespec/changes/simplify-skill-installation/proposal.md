...
```text
0:00  $ ovsespec init
      ✓ Done. Run /ovsx:propose "your idea"

0:15  /ovsx:propose "add user authentication"
...
Power users can customize via `ovsespec config profile`.
...
`/ovsx:propose` should naturally onboard users by explaining what it's doing:
...
When ready to implement, run /ovsx:apply
```
...
Stored in existing global config (`~/.config/ovsespec/config.json`). Not prompted during init.
...
ovsespec config profile core     # preset shortcut (core workflows, preserves delivery)
```
...
**Existing users keep their current setup.** When `ovsespec update` runs on a project with existing workflows and no `profile` in global config, it performs a one-time migration:
...
- `ovsespec init` on a **new** project (no existing workflows) uses global config, defaulting to `core`
...
- Migration message mentions `propose` and suggests `ovsespec config profile core` to opt in
- After migration, users can opt into `core` profile via `ovsespec config profile core`
...
```json
// ~/.config/ovsespec/config.json (extends existing)
{
  "telemetry": { ... },          // existing
...
| custom | user-defined | Pick exactly what you need via `ovsespec config profile` |
