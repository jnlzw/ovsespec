...
The propose workflow SHALL combine change creation and artifact generation into a single command, reducing friction for new users while teaching them the OvseSpec workflow through embedded guidance.
...
- **WHEN** user invokes `/ovsx:propose "add user authentication"`
- **THEN** the system SHALL create a change directory with kebab-case name
- **THEN** the system SHALL create `.ovsespec.yaml` in the change directory (via `ovsespec new change`)
...
- **WHEN** user invokes `/ovsx:propose` with a name that already exists
...
- **WHEN** user invokes `/ovsx:propose "feature name"`
- **THEN** the result SHALL be functionally equivalent to invoking `/ovsx:new "feature-name"` followed by `/ovsx:ff feature-name`
