...
- **WHEN** user runs `ovsespec update`
...
- **WHEN** user runs `ovsespec update`
- **AND** a tool has generated OvseSpec command files
- **AND** that tool has no OvseSpec skill files (commands-only delivery)
...
- **THEN** the system SHALL display: "New in this version: /ovsx:propose (combines new + ff). Try 'ovsespec config profile core' for the streamlined 4-workflow experience."
...
- **WHEN** user runs `ovsespec update` multiple times
...
- **WHEN** user runs `ovsespec update` non-interactively (e.g., in CI)
...
- **THEN** the system SHALL display: "Detected new tool: Windsurf. Run 'ovsespec init' to add it."
...
- **THEN** the system SHALL display one consolidated message listing all detected tools, for example: "Detected new tools: GitHub Copilot, Windsurf. Run 'ovsespec init' to add them."
...
### Requirement: Update requires an OvseSpec project
The update command SHALL only run inside an initialized OvseSpec project.

#### Scenario: Update outside a project
- **WHEN** user runs `ovsespec update`
- **AND** no `ovsespec/` directory exists in the current working directory
- **THEN** the system SHALL display: "No OvseSpec project found. Run 'ovsespec init' to set up."
...
- **WHEN** user runs `ovsespec update`
- **AND** project has workflows not in current profile (e.g., user switched from custom to core or deselected workflows via `ovsespec config profile`)
