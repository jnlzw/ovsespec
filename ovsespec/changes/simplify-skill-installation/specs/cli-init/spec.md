...
- **WHEN** user runs `ovsespec init` interactively and tool directories are detected
...
- **WHEN** user runs `ovsespec init` interactively and no tool directories are detected
- **THEN** the system SHALL prompt for tool selection
- **THEN** the system SHALL use default profile (`core`) and delivery (`both`)

#### Scenario: Non-interactive with detected tools
- **WHEN** user runs `ovsespec init` non-interactively (e.g., in CI)
...
- **WHEN** user runs `ovsespec init` non-interactively
...
- **WHEN** user runs `ovsespec init --tools claude`
...
- **WHEN** user runs `ovsespec init --tools claude` interactively
...
- **THEN** for tools using colon syntax (Claude Code): "Start your first change: /ovsx:propose \"your idea\""
- **THEN** for tools using hyphen syntax (Cursor, others): "Start your first change: /ovsx-propose \"your idea\""
...
- **THEN** for tools using colon syntax: "Start your first change: /ovsx:new \"your idea\""
- **THEN** for tools using hyphen syntax: "Start your first change: /ovsx-new \"your idea\""

#### Scenario: Init success message (neither propose nor new)
- **WHEN** init completes successfully
- **AND** neither `propose` nor `new` is in the active profile
- **THEN** the system SHALL display: "Done. Run 'ovsespec config profile' to configure your workflows."
...
- **WHEN** user runs `ovsespec init` on a project with existing workflow files
...
- **WHEN** user runs `ovsespec init` on a project with no existing workflow files
- **AND** global config does not contain a `profile` field
...
- **WHEN** user runs `ovsespec init --profile core`
...
- **WHEN** user runs `ovsespec init --profile <invalid>`
...
- **WHEN** user runs `ovsespec init` interactively
...
- **WHEN** user has custom profile with extra workflows and runs `ovsespec init` with core profile
...
- **WHEN** user runs `ovsespec init` on existing project
...
- **WHEN** user runs `ovsespec init` on an existing project
