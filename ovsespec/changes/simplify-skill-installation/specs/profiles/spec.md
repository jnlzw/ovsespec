...
- **WHEN** user runs `ovsespec config profile`
...
- **THEN** the system SHALL display: "Config updated. Run `ovsespec update` in your projects to apply."

#### Scenario: Core preset shortcut
- **WHEN** user runs `ovsespec config profile core`
...
- **THEN** the system SHALL NOT modify any project files
- **THEN** the system SHALL display: "Config updated. Run `ovsespec update` in your projects to apply."
- **THEN** the new profile takes effect on the next `ovsespec init` or `ovsespec update` run

#### Scenario: Config profile run inside a project
- **WHEN** user runs `ovsespec config profile` inside an OvseSpec project directory
- **THEN** after updating global config, the system SHALL prompt: "Apply to this project now? (y/n)"
- **WHEN** user confirms
- **THEN** the system SHALL run `ovsespec update` automatically
- **THEN** the system SHALL still display: "Run `ovsespec update` in your other projects to apply."
...
- **WHEN** user runs `ovsespec config profile` non-interactively (e.g., in CI, no TTY)
- **THEN** the system SHALL display an error: "Interactive mode required. Use `ovsespec config profile core` or set config via environment/flags."
- **THEN** the system SHALL exit with code 1

### Requirement: Profile settings stored in global config
Profile and delivery settings SHALL be stored in the existing global config file (`~/.config/ovsespec/config.json`) alongside telemetry and feature flags.
...
- **WHEN** user runs `ovsespec config list`
...
- **WHEN** user updates config via `ovsespec config profile`
- **THEN** the system SHALL only update global config (`~/.config/ovsespec/config.json`)
- **THEN** the system SHALL NOT modify any project skill/command files
- **THEN** existing projects retain their current workflow files until user runs `ovsespec update`

### Requirement: Config changes applied via update command
The existing `ovsespec update` command SHALL apply the current global config to a project. See `specs/cli-update/spec.md` for detailed update behavior.

#### Scenario: Config changes require explicit project sync
- **WHEN** user updates profile or delivery via `ovsespec config profile`
- **THEN** the global config SHALL be updated immediately
- **AND** project files SHALL remain unchanged until `ovsespec update` is run for that project
