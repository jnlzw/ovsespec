...
Currently, there is **no way to regenerate artifacts** in the OVSX workflow:

- `/ovsx:apply` just reads whatever's on disk
- `/ovsx:continue` only creates the NEXT artifact - won't touch existing ones

If you edit `design.md` after `tasks.md` exists, your only options are:
1. Delete tasks.md manually, then run `/ovsx:continue`
...
Add artifact staleness detection to `/ovsx:apply`:

1. **Track modification times**: When generating an artifact, record the mtime of its dependencies
2. **Detect staleness**: When `/ovsx:apply` runs, check if upstream artifacts (design.md, specs) have been modified since tasks.md was generated
3. **Prompt user**: If stale, ask: "Design was modified after tasks were generated. Would you like to regenerate tasks with `/ovsx:continue`?"
...
/ovsx:continue --regenerate tasks

# Option B: Separate command
/ovsx:regenerate tasks
...
Store `.ovsespec-meta.json` in change directory:
```json
{
  "tasks.md": {
