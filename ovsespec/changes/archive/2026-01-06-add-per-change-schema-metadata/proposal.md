...
**Scope: Experimental artifact workflow only** (`ovsespec new change`, `ovsespec status`, `ovsespec instructions`, `ovsespec templates`)

- Store schema choice in `.ovsespec.yaml` metadata file when creating a change via `ovsespec new change`
...
- Add `--schema` option to `ovsespec new change` command

**Not affected**: Legacy commands (`ovsespec validate`, `ovsespec archive`, `ovsespec list`, `ovsespec show`)
...
- **Backward compatible**: Changes without `.ovsespec.yaml` fall back to `spec-driven` default
