...
The metadata file (`.ovsespec.yaml`) will be validated with this Zod schema:
...
/**
 * Schema for per-change metadata stored in .ovsespec.yaml
 */
...
**Location:** `ovsespec/changes/<name>/.ovsespec.yaml`
...
- `ovsespec.json` - YAML matches existing schema files
...
const METADATA_FILENAME = '.ovsespec.yaml';
...
2. **`.ovsespec.yaml` metadata** (persisted choice)
...
No migration needed:
- Existing changes without `.ovsespec.yaml` continue to work (use default)
- New changes created with `ovsespec new change --schema X` get metadata file

## Open Questions

- Should `ovsespec new change` prompt for schema interactively if not specified? (Leaning no - default is fine)
