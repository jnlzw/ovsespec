...
OvseSpec needs a standard location for user-level configuration that works across platforms and follows established conventions. This will serve as the foundation for settings, feature flags, and future artifacts like workflows or templates.
...
```
Unix/macOS: $XDG_CONFIG_HOME/ovsespec/ or ~/.config/ovsespec/
Windows:    %APPDATA%/ovsespec/
```
...
- `~/.ovsespec/` - Simple but clutters home directory
