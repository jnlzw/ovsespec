...
state via `process.cwd()` and `ovsespec/changes`. This makes outcomes depend on
...
- Each suite that touches filesystem discovery creates a temporary directory:
  - `ovsespec/changes/sample-change/proposal.md`
  - `ovsespec/changes/sample-change/specs/sample/spec.md`
...
- Tests that previously depended on repo state now:
  ...
  - Do not read real `ovsespec/changes` during execution.
