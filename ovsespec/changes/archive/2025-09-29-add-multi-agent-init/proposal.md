...
## Summary
- Let `ovsespec init` configure new AI coding tools for projects that already contain an OvseSpec structure.
...
## Motivation
Today `ovsespec init` exits with an error once an `ovsespec/` directory exists. That protects the directory layout, but it blocks
...
## Proposal
1. Detect an existing OvseSpec structure at the start of `ovsespec init` and branch into an "extend" mode instead of exiting.
...
4. Summarize the outcome (created, refreshed, skipped) before exiting with code 0 when work was performed.
   - Include friendly guidance that future updates to shared content still come from `ovsespec update`.

## Out of Scope
- Changing how `ovsespec update` discovers or updates AI tool files.
