...
The `global-config` spec defines how OvseSpec reads/writes `config.json`, but users currently must edit it by hand. This command provides a CLI interface to that config.
...
**Examples:**
```bash
ovsespec config get featureFlags              # Returns object
ovsespec config get featureFlags.experimental # Returns nested value
ovsespec config set featureFlags.newFlag true
```
...
**Rationale:**
- Raw output enables piping: `VAR=$(ovsespec config get key)`
