# Add Init Command for OvseSpec

## Why

Projects need a simple way to adopt OvseSpec conventions. Currently, users must manually create the directory structure and understand all the conventions, which creates friction for adoption. An init command would enable instant OvseSpec setup with proper structure and guidance.

## What Changes

- Add `ovsespec init` CLI command that creates the complete OvseSpec directory structure
...
- Validation to prevent overwriting existing OvseSpec structures
- Clear error messages with helpful guidance (e.g., suggesting 'ovsespec update' for existing structures)
