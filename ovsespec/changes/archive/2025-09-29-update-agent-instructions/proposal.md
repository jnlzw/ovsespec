# Update OvseSpec Agent Instructions

## Why

The current OvseSpec agent instructions need updates to follow best practices for AI assistant instructions (brevity, clarity, removing ambiguity), ensure CLI commands are current with the actual implementation, and properly document the three-stage workflow pattern that agents should follow.
...
- **Complete command documentation** with all current functionality:
  - `ovsespec init [path]` - Initialize OvseSpec in a project
  - `ovsespec list` - List all active changes (default)
  - `ovsespec list --specs` - List all specifications
  - `ovsespec show [item]` - Display change or spec with auto-detection
  - `ovsespec show` - Interactive mode for selection
  - `ovsespec diff [change]` - Show spec differences for a change
  - `ovsespec validate [item]` - Validate changes or specs
  - `ovsespec archive [change]` - Archive completed change after deployment
  - `ovsespec update [path]` - Update OvseSpec instruction files
...
- **Remove deprecated command references** (noun-first patterns like `ovsespec change show`)
- **Add concrete examples** for each command variation
- **Document debugging commands**:
  - `ovsespec show [change] --json --deltas-only` for inspecting deltas
  - `ovsespec validate [change] --strict` for comprehensive validation
...
- **Delta detection debugging**:
  - Use `ovsespec show [change] --json --deltas-only` to inspect parsed deltas
...
- **Spec discovery workflow** - Always check existing specs before creating new ones:
  - Use `ovsespec list --specs` to see all current specs
...
- Affected documentation:
  - `ovsespec/README.md` - Main OvseSpec instructions
  - CLAUDE.md files generated by `ovsespec init` command

Note: This is a tooling/infrastructure change that doesn't require spec updates. When archiving, use `ovsespec archive update-agent-instructions --skip-specs`.