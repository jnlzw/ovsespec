...
- **NEW**: `ovsespec status --change <id>` shows artifact completion state
- **NEW**: `ovsespec next --change <id>` shows artifacts ready to create
- **NEW**: `ovsespec instructions <artifact> --change <id>` outputs enriched template
- **NEW**: `ovsespec templates [--schema <name>]` shows resolved template paths
- **NEW**: `ovsespec new change <name>` creates a new change directory
