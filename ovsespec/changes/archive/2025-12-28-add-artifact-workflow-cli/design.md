...
```
ovsespec status --change <id>
ovsespec next --change <id>
ovsespec instructions <artifact> --change <id>
ovsespec templates [--schema <name>]
...
```bash
ovsespec status --change add-auth   # explicit, works
ovsespec status                      # error: missing --change
```
...
```bash
ovsespec new change add-feature
```

**Rationale:**
- `ovsespec new <name>` is ambiguous (new what?)
- `ovsespec new change <name>` is clear and extensible
- Can add `ovsespec new spec <name>` later if needed
