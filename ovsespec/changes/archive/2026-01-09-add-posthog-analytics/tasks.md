...
- [x] 2.1 Create or extend global config module for `~/.config/ovsespec/config.json`
...
- [x] 3.1 Implement `isTelemetryEnabled()` checking `OVSESPEC_TELEMETRY`, `DO_NOT_TRACK`, and `CI` env vars
...
- [x] 4.3 Display notice text: "Note: OvseSpec collects anonymous usage stats. Opt out: OVSESPEC_TELEMETRY=0"
...
- [x] 6.1 Test opt-out via `OVSESPEC_TELEMETRY=0`
...
- [x] 7.2 Document opt-out methods (`OVSESPEC_TELEMETRY=0`, `DO_NOT_TRACK=1`)
