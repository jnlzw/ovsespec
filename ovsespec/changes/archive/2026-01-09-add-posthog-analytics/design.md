...
OvseSpec needs usage analytics to understand adoption and inform product decisions. PostHog provides a privacy-conscious analytics platform suitable for open source projects.
...
```bash
OVSESPEC_TELEMETRY=0    # Disable telemetry
...
- Full `ovsespec telemetry` command - Can add later if users request
...
```typescript
{
  ...
  properties: {
    ...
    version: '1.2.3'      // OvseSpec version
  }
}
```
...
```
Note: OvseSpec collects anonymous usage stats. Opt out: OVSESPEC_TELEMETRY=0
```
