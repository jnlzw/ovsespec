...
OvseSpec currently resolves schemas from two locations:
1. User override: `~/.local/share/ovsespec/schemas/<name>/`
2. Package built-in: `<npm-package>/schemas/<name>/`

This change adds a third, highest-priority level: project-local schemas at `./ovsespec/schemas/<name>/`.
...
- Schema management CLI commands (`ovsespec schema copy/which/diff/reset`)
...
```
1. ./ovsespec/schemas/<name>/              # Project-local (highest)
2. ~/.local/share/ovsespec/schemas/<name>/ # User override
...
function getProjectSchemasDir(projectRoot: string): string {
  return path.join(projectRoot, 'ovsespec', 'schemas');
}
```
...
A team could create `ovsespec/schemas/spec-driven/` that shadows the built-in, causing confusion when someone expects default behavior.

**Mitigation:** The `ovsespec schemas` command shows the source of each schema. Users can see `spec-driven (project)` vs `spec-driven (package)`.
