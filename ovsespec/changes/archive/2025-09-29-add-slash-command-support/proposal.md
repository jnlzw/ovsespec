...
## Summary
- Enable OvseSpec to generate and update custom slash commands for supported coding agents (Claude Code and Cursor).
- Provide three slash commands aligned with OvseSpec's workflow: proposal (start a change proposal), apply (implement), and archive.
- Share slash command templating between agents to make future extensions simple.

## Motivation
Developers use different coding agents and editors. Having consistent slash commands across tools for the OvseSpec workflow reduces friction and ensures a standard way to trigger the workflow. Supporting both Claude Code and Cursor now lays a foundation for future agents that introduce slash command features.

## Proposal
1. During `ovsespec init`, when a user selects a supported tool, generate slash command configuration for three OvseSpec workflow stages:
   - Claude (namespaced): `/ovsespec/proposal`, `/ovsespec/apply`, `/ovsespec/archive`.
   - Cursor (flat, prefixed): `/ovsespec-proposal`, `/ovsespec-apply`, `/ovsespec-archive`.
   - Semantics:
   ...
   - Each command file MUST embed concise, step-by-step instructions sourced from `ovsespec/README.md` (see Template Content section).
2. Store slash command files per tool:
   - Claude Code: `.claude/commands/ovsespec/{proposal,apply,archive}.md`
   - Cursor: `.cursor/commands/{ovsespec-proposal,ovsespec-apply,ovsespec-archive}.md`
...
3. Command file format and metadata:
   ...
   - Place OvseSpec markers around the body only, never inside frontmatter.
   - Keep the visible slash name, file name, and any frontmatter `name`/`id` consistently aligned (e.g., `proposal`, `ovsespec-proposal`).
   - Namespacing: categorize these under “OvseSpec” and prefer unique IDs (e.g., `ovsespec-proposal`) to avoid collisions.
...
5. During `ovsespec update`, refresh only existing slash command files (per-file basis) within markers; do not create missing files or new tools.
...
- Introduce `SlashCommandConfigurator` to manage multiple files per tool.
  ...
  - Provide `generateAll(projectPath, ovsespecDir)` for init and `updateExisting(projectPath, ovsespecDir)` for update.
- Per-tool adapters add only frontmatter and pathing; bodies come from shared templates.
- Templates live in `TemplateManager` with helpers that extract concise, authoritative snippets from `ovsespec/README.md`.
...
- Markers MUST wrap only the Markdown body contents:
  ...
  - Then `<!-- OVSESPEC:START -->` … body … `<!-- OVSESPEC:END -->`.
...
- Directory creation for `.claude/commands/ovsespec/` and `.cursor/commands/` is the configurator’s responsibility.

### Command Naming & UX
- Claude Code: use namespacing in the slash itself for readability and grouping: `/ovsespec/proposal`, `/ovsespec/apply`, `/ovsespec/archive`.
- Cursor: use flat names with an `ovsespec-` prefix: `/ovsespec-proposal`, `/ovsespec-apply`, `/ovsespec-archive`. Group via `category: OvseSpec` when supported.
- Consistency: align file names, visible slash names, and any frontmatter `id` (e.g., `id: ovsespec-apply`).
...
- Allow users to customize command names and categories during `ovsespec init`.
...
### Claude Code: `.claude/commands/ovsespec/proposal.md`
```markdown
---
name: OvseSpec: Proposal
description: Scaffold a new OvseSpec change and validate strictly.
category: OvseSpec
tags: [ovsespec, change]
---
<!-- OVSESPEC:START -->
...
Slash invocation: `/ovsespec/proposal` (namespaced)

### Cursor: `.cursor/commands/ovsespec-proposal.md`
```markdown
---
name: /ovsespec-proposal
id: ovsespec-proposal
category: OvseSpec
description: Scaffold a new OvseSpec change and validate strictly.
---
<!-- OVSESPEC:START -->
...
Slash invocation: `/ovsespec-proposal` (flat, prefixed)

## Template Content
Templates should be brief, actionable, and sourced from `ovsespec/README.md` to avoid duplication. Each command body includes:
...
- Pointers to `ovsespec show`, `ovsespec list`, and troubleshooting tips when validation fails.
