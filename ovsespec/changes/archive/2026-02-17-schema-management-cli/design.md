...
### 1. Command Structure: `ovsespec schema <subcommand>`

Add a new command group following the existing pattern used by `ovsespec config` and `ovsespec completion`.
...
- Flat commands (`ovsespec schema-init`, `ovsespec schema-fork`): Rejected because it pollutes the top-level namespace and doesn't scale well.
- Extending existing commands (`ovsespec init --schema`): Rejected because schema management is distinct from project initialization.
...
**Rationale:** Matches the UX established by `ovsespec init` and `ovsespec config reset`. Provides a guided experience while keeping the wizard lightweight.
...
The destination is always project-local: `ovsespec/schemas/<name>/`
...
**Rationale:** Consistent with existing OvseSpec commands and enables scripting/automation.
