...
Add a new `ovsespec schema` command group with subcommands for creating, forking, validating, and inspecting schemas.

### Commands

1. **`ovsespec schema init <name>`** - Interactive wizard to scaffold a new project schema
   ...
   - Optionally sets as project default in `ovsespec/config.yaml`

2. **`ovsespec schema fork <source> [name]`** - Copy an existing schema as a starting point
...
3. **`ovsespec schema validate [name]`** - Validate schema structure and templates
...
4. **`ovsespec schema which <name>`** - Show schema resolution path
