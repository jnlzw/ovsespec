...
2. Add `--specs`/`--changes` to `ovsespec list`, with `--changes` as default.
3. Show deprecation warnings for `ovsespec change list` and, more generally, for any `ovsespec change ...` and `ovsespec spec ...` subcommands.
...
## Out of Scope
JSON output parity for `ovsespec list` across modes and `show --specs/--changes` discovery are follow-ups.
