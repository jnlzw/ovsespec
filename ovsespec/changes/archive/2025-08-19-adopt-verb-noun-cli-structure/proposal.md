...
- Deprecate noun-based top-level commands: `ovsespec spec ...` and `ovsespec change ...`.
...
- **List**
  - From: `ovsespec change list`
  - To: `ovsespec list --changes` (default), or `ovsespec list --specs`

- **Show**
  - From: `ovsespec spec show <spec-id>` / `ovsespec change show <change-id>`
  - To: `ovsespec show <item-id>` with auto-detect, use `--type spec|change` if ambiguous

- **Validate**
  - From: `ovsespec spec validate <spec-id>` / `ovsespec change validate <change-id>`
  - To: `ovsespec validate <item-id> --type spec|change`, or bulk: `ovsespec validate --specs` / `--changes` / `--all`

### Backward Compatibility

- Keep `ovsespec spec` and `ovsespec change` available with deprecation warnings for one release cycle.
...
- **Affected specs**:
  ...
  - `ovsespec-conventions`: Add explicit requirement establishing verb–noun CLI design and deprecation guidance
- **Affected code**:
...
**CLI Design**
- From: Mixed model with nouns (`spec`, `change`) and some top-level verbs; `ovsespec list` currently deprecated
- To: Verbs as primary: `ovsespec list|show|validate|diff|archive`; nouns scoped via flags or item ids; noun commands deprecated
...
**Listing Behavior**
- From: `ovsespec change list` (primary), `ovsespec list` (deprecated)
- To: `ovsespec list` as primary, defaulting to `--changes`; add `--specs` to list specs
...
- Document new usage in `ovsespec/README.md` and CLI help.
