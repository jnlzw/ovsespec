...
Currently `ovsespec init` and `ovsespec experimental` are separate commands with distinct purposes:

- **init**: Creates `ovsespec/` directory, generates `AGENTS.md`/`project.md`, configures tool config files (`CLAUDE.md`, etc.), generates old slash commands (`/ovsespec:proposal`, etc.)
- **experimental**: Generates skills (9 per tool), generates ovsx slash commands (`/ovsx:new`, etc.), creates `config.yaml`
...
**Goals:**
- Single `ovsespec init` command that sets up the complete skill-based workflow
...
- Backward compatibility for `/ovsespec:*` commands (breaking change)
...
**Choice**: For files with mixed content (OvseSpec markers + user content), only remove the OvseSpec marker block. For files that are 100% OvseSpec content, delete the entire file.

**Rationale**: Respects user customizations. CLAUDE.md might have other instructions beyond OvseSpec.

**Edge cases**:
- **Config files with mixed content**: Remove only `<!-- OVSESPEC:START -->` to `<!-- OVSESPEC:END -->` block
- **Config files that are 100% OvseSpec**: Delete file entirely (check if content outside markers is empty/whitespace)
- **Old slash command directories** (`.claude/commands/ovsespec/`): Delete entire directory (ours)
- **`ovsespec/AGENTS.md`**: Delete (ours)
- **Root `AGENTS.md`**: Only remove OvseSpec marker block, preserve rest

### Decision 6: Preserve project.md with migration hint

**Choice**: Do NOT auto-delete `ovsespec/project.md`. Preserve it and show a message directing users to manually migrate content to `config.yaml`'s `context:` field.
...
- Users can migrate manually or use `/ovsx:explore` to get AI assistance

**Migration path**:
1. During legacy cleanup, detect `ovsespec/project.md` but do not delete
2. Show in output: "ovsespec/project.md still exists - migrate content to config.yaml's context: field, then delete"
...
**Choice**: Keep `ovsespec experimental` as a hidden command that delegates to `init`.
...
| Risk | Mitigation |
|------|------------|
| Users with custom `/ovsespec:*` commands lose them | Document in release notes; old commands are in git history |
...
| `ovsespec update` might break | Review and update `update` command to work with new structure |
...
.<tool>/commands/ovsx/      # 9 slash commands per selected tool
  ├── explore.md
  ├── new.md
  ├── continue.md
  ├── apply.md
...
- `ovsespec/project.md`
- Root `AGENTS.md` stub
- `.claude/commands/ovsespec/` (old slash commands)

### Legacy detection targets

| Artifact Type | Detection Method | Removal Method |
|--------------|------------------|----------------|
| Config files (CLAUDE.md, etc.) | File exists AND contains OvseSpec markers | Remove marker block; delete file if empty after |
| Old slash command dirs | Directory exists at `.<tool>/commands/ovsespec/` | Delete entire directory |
| ovsespec/AGENTS.md | File exists at `ovsespec/AGENTS.md` | Delete file |
| ovsespec/project.md | File exists at `ovsespec/project.md` | **Preserve** - show migration hint only |
| Root AGENTS.md | File exists at `AGENTS.md` AND contains OvseSpec markers | Remove marker block; delete file if empty after |
...
- `src/core/templates/slash-command-templates.ts` - old `/ovsespec:*` content
...
1. **What happens to `ovsespec update`?** - RESOLVED

   **Current behavior**: Updates `ovsespec/AGENTS.md`, config files (`CLAUDE.md`, etc.) via `ToolRegistry`, and old slash commands (`/ovsespec:*`) via `SlashCommandRegistry`.

   **New behavior**: Rewrite to refresh skills and ovsx commands instead:
   - Detect which tools have skills installed (check for `.claude/skills/ovsespec-*/`, etc.)
   - Refresh all 9 skill files per installed tool using `skill-templates.ts`
   - Refresh all 9 ovsx command files per installed tool using `command-generation/` adapters
...
2. **Should we keep `ovsespec schemas` and other experimental subcommands?** - RESOLVED
