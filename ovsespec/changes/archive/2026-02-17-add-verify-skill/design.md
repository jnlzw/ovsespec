# Design: Add /ovsx:verify Skill

## Architecture Decision: Dynamic Generation via Setup Command

### Context

All existing ovsx experimental skills (explore, new, continue, apply, ff, sync, archive) are dynamically generated when users run `ovsespec artifact-experimental-setup`. They are not manually created files checked into the repository.
...
1. **Consistency**: All 7 existing ovsx skills follow this pattern. Adding verify as the 8th skill should follow the same architecture.
...
3. **Distribution**: Users get the verify skill automatically when running `ovsespec artifact-experimental-setup`, just like all other ovsx skills. No special installation steps needed.
...
export function getOvsxVerifyCommandTemplate(): CommandTemplate
```
...
- Update help text to list `/ovsx:verify`

#### 3. Generated Artifacts

When users run `ovsespec artifact-experimental-setup`, the command creates:

- `.claude/skills/ovsespec-verify-change/SKILL.md` - Agent Skills format
- `.claude/commands/ovsx/verify.md` - Slash command format
...
Create `.claude/skills/ovsespec-verify-change/SKILL.md` as a static file in the OvseSpec repository.

**Rejected because:**
- Inconsistent with all other ovsx skills
...
Add `ovsespec setup-verify` as a separate command.
...
3. Help text mentions `/ovsx:verify`
4. Running `ovsespec artifact-experimental-setup` generates both skill and command files
