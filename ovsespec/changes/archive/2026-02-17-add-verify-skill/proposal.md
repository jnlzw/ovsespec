...
- Add `getOvsxVerifyCommandTemplate()` function to `skill-templates.ts`
...
- Update help text to include `/ovsx:verify` in the list of available commands
- Create `ovsx-verify-skill` capability spec
...
- Affected specs: New `ovsx-verify-skill` spec
- Affected code:
...
- Generated artifacts: When users run `ovsespec artifact-experimental-setup`:
  - Creates `.claude/skills/ovsespec-verify-change/SKILL.md`
  - Creates `.claude/commands/ovsx/verify.md`
- Related skills: Works alongside `/ovsx:apply` and before `/ovsx:archive`
