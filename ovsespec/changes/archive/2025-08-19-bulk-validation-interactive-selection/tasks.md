...
- [x] 1.2 Implement interactivity gate respecting TTY and `OVSE_SPEC_INTERACTIVE=0` in `src/commands/change.ts`
...
- [x] 2.3 Implement interactivity gate respecting TTY and `OVSE_SPEC_INTERACTIVE=0`
- [x] 2.4 When no `[spec-id]` provided and interactivity allowed, prompt to select from `ovsespec/specs/*/spec.md` and validate the selected spec
...
- [x] 3.1 Add `validate` command in `src/cli/index.ts`
  ...
  - Usage: `ovsespec validate [item-name]`
- [x] 3.2 Create `src/commands/validate.ts` implementing:
  ...
  - [x] 3.2.6 Bulk validation for `--all`, `--changes`, `--specs` (exclude `ovsespec/changes/archive/`)
...
- [x] 4.1 Add `src/utils/interactive.ts` with `isInteractive(stdin: NodeJS.ReadStream, noInteractiveFlag?: boolean): boolean`
  - Considers: `process.stdin.isTTY`, `--no-interactive`, `OVSE_SPEC_INTERACTIVE=0`
...
- [x] 8.3 Ensure existing specs under `ovsespec/changes/bulk-validation-interactive-selection/specs/*` remain satisfied
...
- [x] AC-1: `ovsespec change validate` interactive selection when no arg (TTY only; respects `--no-interactive`/env) — matches cli-change spec
- [x] AC-2: `ovsespec spec validate` interactive selection when no arg (TTY only; respects `--no-interactive`/env) — matches cli-spec spec
- [x] AC-3: New `ovsespec validate` supports interactive selection, bulk/filtered validation, JSON schema, progress, concurrency, exit codes — matches cli-validate spec
