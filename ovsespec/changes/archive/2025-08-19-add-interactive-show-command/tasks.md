...
- Respect `--no-interactive` and `OVSE_SPEC_INTERACTIVE=0` consistently.
...
Acceptance:
- `ovsespec show` exists and prints a helpful hint in non-interactive contexts when no args.
...
Acceptance:
- `ovsespec show <change-id> --json --deltas-only` matches `ovsespec change show <id> --json --deltas-only` output.
- `ovsespec show <spec-id> --json --requirements` matches `ovsespec spec show <id> --json --requirements` output.
...
- [x] `src/commands/change.ts` → extend `show(changeName?: string, options?: { json?: boolean; requirementsOnly?: boolean; deltasOnly?: boolean; noInteractive?: boolean })`:
  ...
  - Non-interactive fallback: keep current behavior (print available IDs + `ovsespec change list` hint, set `process.exitCode = 1`).
...
Acceptance:
- `ovsespec change show` in non-interactive prints list hint and exits non-zero.
- `ovsespec spec show` in non-interactive prints missing-arg error and exits non-zero.
...
- [x] Top-level `show` hint (non-interactive no-arg):
  - Lines include: `ovsespec show <item>`, `ovsespec change show`, `ovsespec spec show`, and "Or run in an interactive terminal.".
...
## 7) Tests
Add tests mirroring existing patterns (non-TTY simulation via `OVSE_SPEC_INTERACTIVE=0`).
...
- [x] `test/commands/change.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `ovsespec change show` without args prints available IDs + list hint and non-zero exit.
- [x] `test/commands/spec.interactive-show.test.ts` (non-interactive fallback)
  - Ensure `ovsespec spec show` without args prints missing-arg error and non-zero exit.
...
- [x] Update `ovsespec/README.md` usage examples to include the new `show` command with type detection and flags.
