...
The `ovsespec diff` command adds unnecessary complexity to the OvseSpec CLI for several reasons:

1. **Redundant functionality**: The `ovsespec show` command already provides comprehensive visualization of changes through structured JSON output and markdown rendering
...
Remove the `ovsespec diff` command entirely and guide users to more appropriate alternatives:

1. **For viewing change content**: Use `ovsespec show <change-name>` which provides:
...
- **Better alignment**: Focuses on the core OvseSpec workflow without redundant features

## Implementation

### Files to Remove
- `/src/core/diff.ts` - The entire diff command implementation
- `/ovsespec/specs/cli-diff/spec.md` - The diff command specification
...
- `/ovsespec/README.md` - Remove diff command references
- Various documentation files mentioning `ovsespec diff`

### Migration Guide for Users

Users currently using `ovsespec diff` should transition to:

```bash
# Before
ovsespec diff add-feature

# After - view the change proposal
ovsespec show add-feature

# After - view only the deltas
ovsespec show add-feature --json --deltas-only

# After - use git for file comparisons
git diff ovsespec/specs ovsespec/changes/add-feature/specs
```
