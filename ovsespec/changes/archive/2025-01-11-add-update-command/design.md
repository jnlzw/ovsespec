...
- Full replacement for OvseSpec-managed files only (e.g., `ovsespec/README.md`)
...
- Use existing template utilities
  - `readmeTemplate` from `src/core/templates/readme-template.ts` for `ovsespec/README.md`
  - `TemplateManager.getClaudeTemplate()` for `CLAUDE.md`
- Directory name is fixed to `ovsespec` (from `OVSESPEC_DIR_NAME`)

### File Operations
- Use async utilities for consistency
  - `FileSystemUtils.writeFile` for `ovsespec/README.md`
...
export class UpdateCommand {
  async execute(projectPath: string): Promise<void> {
    const ovsespecDirName = OVSESPEC_DIR_NAME;
    const ovsespecPath = path.join(projectPath, ovsespecDirName);
    ...
    const readmePath = path.join(ovsespecPath, 'README.md');
    ...
    console.log('Updated OvseSpec instructions');
  }
}
```
...
- Full overwrite only for OvseSpec-managed files
- Marker-managed updates for user-owned files

## Error Handling

Only handle critical errors:
- Missing `ovsespec` directory → throw error handled by CLI to present a friendly message
- File write failures → let errors bubble up to CLI

## Testing Strategy

Manual smoke tests are sufficient initially:
1. Run `ovsespec init` in a test project
2. Modify both files (including custom content around markers in `CLAUDE.md`)
3. Run `ovsespec update`
4. Verify `ovsespec/README.md` fully replaced; `CLAUDE.md` OvseSpec block updated without altering user content outside markers
5. Run the command twice to verify idempotency and no duplicate markers
6. Test with missing `ovsespec` directory (expect failure)