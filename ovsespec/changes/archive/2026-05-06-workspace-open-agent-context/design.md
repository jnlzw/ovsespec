...
workspace links = the repos or folders OvseSpec can plan across
...
```bash
ovsespec workspace open platform
ovsespec workspace open --workspace platform
```

User-facing docs should prefer the positional form. If both are provided and they differ, OvseSpec should fail with a clear conflict error.
...
This slice should not add global workspace opener config. OvseSpec already has a global config system, and workspace-level defaults can be added there later if repeated setup makes the local prompt feel noisy.
...
Store the preferred opener as a structured object in `.ovsespec-workspace/local.yaml`:
...
```bash
ovsespec workspace setup --no-interactive --name platform --link /repo --opener codex
ovsespec workspace setup --no-interactive --name platform --link /repo --opener editor
```
...
If OvseSpec later supports a Copilot CLI agent, it should use a distinct value such as `github-copilot-cli` and launch the CLI agent directly. VS Code Copilot and a CLI agent have different opener mechanics, so they should remain distinct opener values.
...
For VS Code-style editor support, OvseSpec should create and maintain a `.code-workspace` file as part of the workspace setup/link/relink lifecycle. `workspace open` should launch against existing workspace state.
...
The `.code-workspace` file should include the workspace root and each linked repo or folder with a valid local path. Because linked paths come from machine-local workspace state, OvseSpec-created workspaces should ignore the maintained `.code-workspace` file by default.
...
For Claude and Codex, `workspace open` may still need to pass workspace and linked directory arguments to the agent process at launch because those tools do not consume `.code-workspace` directly. If an opener requires an initial prompt argument, it should be minimal, such as `Open this OvseSpec workspace.`

Dynamic workspace facts should normally be discoverable from existing files:

- linked paths: `.ovsespec-workspace/local.yaml`
- stable link names: `.ovsespec-workspace/workspace.yaml`
...
OvseSpec should own a marked workspace-guidance block inside `AGENTS.md`:
...
...
<!-- OVSESPEC:WORKSPACE-GUIDANCE:END -->
```
...
If `AGENTS.md` is missing, OvseSpec should recreate it. If `AGENTS.md` exists and the markers are absent, OvseSpec should append the managed block while preserving existing content.

## Linked Paths

Root workspace open should attach every linked repo or folder with a valid local path.

Broken links are skipped during workspace open. OvseSpec should surface clear status in human output, with `ovsespec workspace doctor` as the repair path.

Links with repo-local `ovsespec/` state absent remain valid for workspace open. Missing repo-local OvseSpec state can matter later for implementation readiness while still allowing visibility for exploration and planning.
