...
- [x] 1.2 Generate `.agent/workflows/ovsespec-proposal.md`, `ovsespec-apply.md`, and `ovsespec-archive.md` with YAML frontmatter restricted to a single `description` field for each stage and wrap the body in OvseSpec markers.
...
- [x] 2.1 Detect existing Antigravity workflow files during `ovsespec update` and refresh only the managed body, skipping creation when files are missing.
