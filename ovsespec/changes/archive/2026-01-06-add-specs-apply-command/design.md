...
Currently, delta specs are only applied to main specs when running `ovsespec archive`. This bundles two concerns:
...
- Provide `/ovsx:sync` skill for agents to sync specs on demand
...
**Alternatives considered**:
- Track `specsSynced: true` in `.ovsespec.yaml` (rejected: unnecessary complexity)
- Store snapshot of synced deltas (rejected: over-engineering)

### 3. Agent-driven approach (no CLI command)

**Decision**: The `/ovsx:sync` skill is fully agent-driven - the agent reads delta specs and directly edits main specs.
...
**Rationale**: Backward compatibility. Users who don't use `/ovsx:sync` get the same experience.
...
→ Running `/ovsx:sync` again reconciles. Idempotent design handles this.
...
2. Add skill template for `/ovsx:sync` in `skill-templates.ts`
