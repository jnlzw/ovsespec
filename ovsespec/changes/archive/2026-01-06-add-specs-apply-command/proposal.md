...
Spec application is currently bundled with archive - users must run `ovsespec archive` to apply delta specs to main specs. This couples two distinct concerns (applying specs vs. archiving the change) and forces users to wait until they're "done" to see main specs updated. Users want the flexibility to sync specs earlier in the workflow while iterating.

## What Changes

- Add `/ovsx:sync` skill that syncs delta specs to main specs as a standalone action
...
**Workflow becomes:**
```
/ovsx:new → /ovsx:continue → /ovsx:apply → archive
                                  │
                                  └── /ovsx:sync (optional, anytime)
```

## Capabilities

### New Capabilities
- `specs-sync-skill`: Skill template for `/ovsx:sync` command that reconciles main specs with delta specs
...
- **Skills**: New `ovsespec-sync-specs` skill in `skill-templates.ts`
