...
OvseSpec users on NixOS or using the Nix package manager cannot easily install or run OvseSpec without going through npm. Adding a Nix flake makes OvseSpec a first-class citizen in the Nix ecosystem, enabling users to run `nix run github:Fission-AI/OvseSpec -- init` or include OvseSpec in their development environments declaratively.
...
### New Capabilities
- `nix-flake-support`: Nix flake configuration for building and running OvseSpec
