...
The system SHALL provide a Nix flake that builds OvseSpec for multiple platforms.

#### Scenario: Build on Linux x86_64
- **WHEN** user runs `nix build` on x86_64-linux system
- **THEN** system builds OvseSpec package successfully
- **AND** package includes the `ovsespec` binary
...
The system SHALL allow users to run OvseSpec directly from GitHub without installing.

#### Scenario: Run init command from GitHub
- **WHEN** user runs `nix run github:Fission-AI/OvseSpec -- init`
- **THEN** system downloads and builds OvseSpec
- **AND** executes `ovsespec init` command

#### Scenario: Run any OvseSpec command
- **WHEN** user runs `nix run github:Fission-AI/OvseSpec -- <command> <args>`
- **THEN** system executes `ovsespec <command> <args>`

### Requirement: pnpm dependency management
The system SHALL use pnpm for building OvseSpec in the Nix flake.
...
- **WHEN** user runs `nix develop` in OvseSpec repository
