## 1. CLI wiring
- [x] 1.1 Add Kilo Code to the selectable AI tools in `ovsespec init`, including "already configured" detection and success summaries.
- [x] 1.2 Register a `KiloCodeSlashCommandConfigurator` alongside other slash-command tools.

## 2. Workflow generation
- [x] 2.1 Implement the configurator so it creates `.kilocode/workflows/` (if needed) and writes `ovsespec-{proposal,apply,archive}.md` with OvseSpec markers.
...
## 3. Update support
- [x] 3.1 Ensure `ovsespec update` refreshes existing Kilo Code workflows while skipping ones that are absent.
