...
OvseSpec currently has a fixed schema resolution order:
1. `--schema` CLI flag
2. `.ovsespec.yaml` in change directory
...
The proposal introduces `ovsespec/config.yaml` as a lightweight customization layer that sits between preset schemas and full forking. It allows teams to:
...
**Key stakeholders:**
- OvseSpec users who need light customization without forking
- Teams sharing workflow conventions via committed config

## Goals / Non-Goals

**Goals:**
- Load and parse `ovsespec/config.yaml` using Zod schema
...
- Config management commands (`ovsespec config init`) - manual creation for now
...
- Consistent with `.ovsespec.yaml` used in changes
...
### 2. Config Location: Project Root vs ovsespec/ Directory

**Decision:** `./ovsespec/config.yaml` (inside ovsespec directory)

**Rationale:**
- Co-located with `ovsespec/schemas/` (project-local-schemas)
- Keeps project root clean
- Natural namespace for OvseSpec configuration
- Mirrors structure used by other tools (e.g., `.github/`)

**Alternatives considered:**
- `./ovsespec.config.yaml` in root: Pollutes root, less clear ownership
...
3. **`ovsespec/config.yaml` schema field** (NEW - project default)
...
**Rationale:**
- Syntax errors in config shouldn't break all of OvseSpec
- User can fix config incrementally
- Commands remain usable during config development

**Warning message:**
```
⚠️  Failed to parse ovsespec/config.yaml: [error details]
 ...
 * - Helps understand OvseSpec's data interfaces at a glance
 * - Single source of truth for type and validation
 * - Consistent with other OvseSpec schemas
 */
...
/**
 * Read and parse ovsespec/config.yaml from project root.
...
export function readProjectConfig(): ProjectConfig | null {
  ...
  let configPath = path.join(projectRoot, 'ovsespec', 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, 'ovsespec', 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`⚠️  ovsespec/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`⚠️  Failed to parse ovsespec/config.yaml:`, error);
    return null;
  }
}
...
export function suggestSchemas(
  ...
  let message = `❌ Schema '${invalidSchemaName}' not found in ovsespec/config.yaml\n\n`;
  ...
  message += `\nFix: Edit ovsespec/config.yaml and change 'schema: ${invalidSchemaName}' to a valid schema name`;
...
```typescript
// Example: "ovsespec instructions proposal --change my-feature"
...
export function readProjectConfig(): ProjectConfig | null {
  const projectRoot = findProjectRoot();
  const configPath = path.join(projectRoot, 'ovsespec', 'config.yaml');
...
┌──────────────────────────────────────────────────────────────┐
│                                                              │
│  User runs: ovsespec instructions proposal --change foo     │
│                                                              │
└────────────────────────────┬─────────────────────────────────┘
                             │
                             ▼
...
│                                                              │
│  1. Check CLI flag ✗                                         │
│  2. Check .ovsespec.yaml ✗                                   │
│  3. Check ovsespec/config.yaml ✓ → "spec-driven"             │
│                                                              │
└────────────────────────────┬─────────────────────────────────┘
                             │
                             ▼
...
**[Risk]** YAML parsing errors break OvseSpec commands
...
→ **Mitigation:** Document common artifact IDs, show in `ovsespec status` output
