...
Add `ovsespec/config.yaml` support for project-level configuration. This enables teams to customize OvseSpec behavior without forking schemas, by providing context and rules that are injected into artifact generation.

## Motivation

Currently, customizing OvseSpec requires forking entire schemas:
- Must copy all files even to add one rule
- Lose updates when ovsespec upgrades
...
OvseSpec customization follows two distinct paths:
...
│   + add rules                                                   │
│                                                                 │
│   ovsespec/config.yaml            ovsespec/schemas/my-flow/     │
│                                                                 │
│   ✓ Simple                        ✓ Full control                │
...
- Commands run on changes without `.ovsespec.yaml` metadata
...
3. **Avoids premature API surface** - No standalone `ovsespec config init` until feature graduates
...
```
Today (Experimental):
  ovsespec artifact-experimental-setup
    → prompts for config creation
    → creates .claude/skills/
    → creates ovsespec/config.yaml

Future (When graduating):
  ovsespec init
    → prompts for config creation
    → creates ovsespec/ directory
    → creates ovsespec/config.yaml

  + standalone commands:
    ovsespec config init
    ovsespec config validate
    ovsespec config set <key> <value>
```

**Why optional?**

Config is **additive**, not required:
- OvseSpec works without config (uses defaults)
...
- Update `ovsespec new change` to use config's schema as default
...
- Integration with standard `ovsespec init` (will add when experimental graduates)
- Standalone `ovsespec config init` command (may add in future change)
- `ovsespec config validate` command (may add in future change)
...
✓ Created .claude/skills/ovsespec-explore/SKILL.md
✓ Created .claude/skills/ovsespec-new-change/SKILL.md
✓ Created .claude/skills/ovsespec-continue-change/SKILL.md
✓ Created .claude/skills/ovsespec-apply-change/SKILL.md
✓ Created .claude/skills/ovsespec-ff-change/SKILL.md
✓ Created .claude/skills/ovsespec-sync-specs/SKILL.md
✓ Created .claude/skills/ovsespec-archive-change/SKILL.md

✓ Created .claude/commands/ovsx/explore.md
✓ Created .claude/commands/ovsx/new.md
✓ Created .claude/commands/ovsx/continue.md
✓ Created .claude/commands/ovsx/apply.md
✓ Created .claude/commands/ovsx/ff.md
✓ Created .claude/commands/ovsx/sync.md
✓ Created .claude/commands/ovsx/archive.md

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📋 Project Configuration (Optional)

Configure project defaults for OvseSpec workflows.

? Create ovsespec/config.yaml? (Y/n) Y
...
✓ Created ovsespec/config.yaml

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🎉 Setup Complete!

📖 Config created at: ovsespec/config.yaml
...
To share with team:
  git add ovsespec/config.yaml .claude/
  git commit -m "Setup OvseSpec experimental workflow with project config"
...
/ovsx:new my-feature --schema spec-driven
...
/ovsx:new my-feature --schema tdd
...
ovsespec instructions proposal --change my-feature
...
ovsespec instructions specs --change my-feature
...
```bash
ovsespec instructions design --change my-feature
...
Always at `./ovsespec/config.yaml` relative to project root. No XDG/global config for simplicity.
...
2. .ovsespec.yaml in change directory   # Change-specific binding
3. ovsespec/config.yaml schema field    # Project default (NEW)
...
6. **Create and validate config:**
   ...
   - Write to `ovsespec/config.yaml` using YAML serializer
...
When `ovsespec/config.yaml` already exists:
...
ℹ️  ovsespec/config.yaml already exists. Skipping config creation.

   To update config, edit ovsespec/config.yaml manually or:
   1. Delete ovsespec/config.yaml
   2. Run ovsespec artifact-experimental-setup again
...
try {
  ...
  if (configResult.createConfig) {
    ...
    console.log('✓ Created ovsespec/config.yaml');
  }
} catch (error) {
...
1. **@inquirer/prompts** (recommended)
   ...
   - Already used in OvseSpec (if not, lightweight addition)
...
- Verify change's `.ovsespec.yaml` overrides config
...
- Test all four levels of schema resolution:
  ...
  2. Change metadata `.ovsespec.yaml`
...
- Existing changes with `.ovsespec.yaml` metadata aren't affected by config
