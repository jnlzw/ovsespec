...
Result: `ovsespec new change foo && ovsespec view` shows `foo` as "Completed" when it has no content.
...
Result: `ovsespec status --change foo` says "not found" even though the directory exists.
...
|---------|--------|-------|
| `ovsespec view` | Empty = "Completed" | Empty = "Draft" |
| `ovsespec status --change X` | Requires proposal.md | Works on any directory |
| `ovsespec validate X` | Requires proposal.md | Unchanged (still requires it) |

## Breaking Changes

### Minimal Breaking Change

1. **`ovsespec view` output**: Empty changes move from "Completed" section to new "Draft" section

### Non-Breaking

- Active/Completed semantics unchanged (still task-based)
- `getActiveChangeIds()` unchanged
- `ovsespec validate` unchanged
...
|---------|-----------------------------|-----------------------------|----------------|
| `ovsespec view` | No (reads dirs directly) | Yes → Draft section | **Yes** |
| `ovsespec list` | No (reads dirs directly) | Yes (shows "No tasks") | No |
| `ovsespec status/next/instructions` | Yes | Yes | **Yes** |
| `ovsespec validate` | Yes | No (can't validate empty) | No |
| `ovsespec show` | Yes | No (nothing to show) | No |
| Tab completions | Yes | Future enhancement | No |

## Success Criteria

1. `ovsespec new change foo && ovsespec view` shows `foo` in "Draft" section
2. `ovsespec new change foo && ovsespec status --change foo` works
