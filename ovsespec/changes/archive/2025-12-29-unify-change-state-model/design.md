...
|--------|---------|---------|
| **Task Progress** | Track implementation work | `ovsespec view`, `ovsespec list` |
| **Artifact Progress** | Track planning/spec work | `ovsespec status`, `ovsespec next` |
...
async function validateChangeExists(changeName: string, projectRoot: string): Promise<string> {
  ...
  if (!fs.existsSync(changePath) || !fs.statSync(changePath).isDirectory()) {
    ...
    const entries = await fs.promises.readdir(
      path.join(projectRoot, 'ovsespec', 'changes'),
      { withFileTypes: true }
    );
...
```bash
# Before
$ ovsespec new change foo
$ ovsespec status --change foo
