...
OvseSpec's existing architecture can already represent that shape:

- `AI_TOOLS` can advertise a `skillsDir`
- `init` can install skills for any selected tool with `skillsDir`
- when command generation is attempted for a tool without an adapter, OvseSpec already records `commandsSkipped`
...
- adding a fake `.kimi/commands/...` path would create behavior OvseSpec cannot justify against upstream Kimi CLI behavior

### 3. Document Kimi by its real invocation surface

Kimi documentation in OvseSpec must use Kimi's actual skill invocation form:

- supported-tools: no generated command files, use `/skill:ovsespec-*`
- commands doc: examples such as `/skill:ovsespec-propose`

The docs must not claim generated `ovsx-*` files or `/ovsespec-*` direct invocations for Kimi.
