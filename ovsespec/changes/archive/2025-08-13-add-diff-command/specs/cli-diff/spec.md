...
The `ovsespec diff` command provides developers with a visual comparison between proposed spec changes and the current deployed specs.

## Command Syntax

```bash
ovsespec diff [change-name]
```
...
WHEN changes directory doesn't exist THEN display "No OvseSpec changes directory found"

## Examples

```bash
# View diff for specific change
$ ovsespec diff add-auth-feature
