...
- [x] Create `src/core/completions/command-registry.ts` with `COMMAND_REGISTRY` constant defining all OvseSpec commands, flags, and metadata
...
- [x] Implement installation to `~/.oh-my-zsh/custom/completions/_ovsespec` for Oh My Zsh
- [x] Implement fallback installation to `~/.zsh/completions/_ovsespec` with `fpath` updates
...
- [x] Implement dynamic completion helper functions in Zsh generator (`_ovsespec_complete_changes`, `_ovsespec_complete_specs`, `_ovsespec_complete_items`)
...
- [ ] Test dynamic change/spec ID completion in real OvseSpec projects
- [ ] Verify completion cache behavior (2-second TTL)
- [ ] Test behavior outside OvseSpec projects (should skip dynamic completions)
- [x] Update `ovsespec --help` output to include completion command (automatically done via Commander)
