...
- **WHEN** user executes `ovsespec completion zsh`
...
- **THEN** remove `~/.oh-my-zsh/custom/completions/_ovsespec` if Oh My Zsh is detected
...
- **WHEN** `ovsespec completion install` cannot detect current shell or detects non-Zsh shell
- **THEN** display error: "Could not detect Zsh. Please specify explicitly: ovsespec completion install zsh"
- **AND** exit with code 1
...
- **AND** allow redirection to files: `ovsespec completion zsh > /path/to/_ovsespec`
