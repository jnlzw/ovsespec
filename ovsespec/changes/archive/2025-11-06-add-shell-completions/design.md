...
**OvseSpec Integration:**
```bash
# After installing: ovsespec completion install bash
ovsespec val<TAB>           # Completes to "ovsespec validate"
ovsespec validate <TAB><TAB>  # Shows: --all --changes --specs --strict --json [change-ids] [spec-ids]
ovsespec show add-<TAB><TAB>  # Shows all changes starting with "add-"
```
...
**OvseSpec Integration:**
```zsh
# After installing: ovsespec completion install zsh
ovsespec val<TAB>    # Shows menu with "validate" and "view" highlighted
ovsespec show <TAB>  # Shows menu with all change IDs and spec IDs, categorized
```
...
**OvseSpec Integration:**
```fish
# After installing: ovsespec completion install fish
ovsespec val       # Gray suggestion shows "validate" immediately
ovsespec show a    # Real-time suggestions for changes starting with "a"
ovsespec <TAB>     # Shows all commands with descriptions in paged menu
```
...
**OvseSpec Integration:**
```powershell
# After installing: ovsespec completion install powershell
ovsespec val<TAB>       # Cycles: validate → view → validate
ovsespec show <TAB>     # Cycles through change IDs one by one
ovsespec <Ctrl+Space>   # Shows IntelliSense menu with all commands
```
...
class CompletionProvider {
  ...
  isOvseSpecProject(): boolean {
    // Check for ovsespec/ directory
  }
}
```
...
- Graceful degradation when outside OvseSpec projects
...
│       ├── command-registry.ts    # Single source of truth for OvseSpec commands
...
if (isOhMyZshInstalled()) {
  // Install to ~/.oh-my-zsh/custom/completions/_ovsespec
  // Automatically loaded by Oh My Zsh
} else {
  // Install to ~/.zsh/completions/_ovsespec
...
2. **Not in OvseSpec project** - Skip dynamic completions, only offer static commands
...
3. **Manual Testing**
   ...
   - Tab completion behavior in OvseSpec projects
