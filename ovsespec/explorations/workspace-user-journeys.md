...
This document describes the exact user experience for OvseSpec across:
...
- How OvseSpec decides which specs to read
- How OvseSpec behaves when roots are missing, ambiguous, or partially available
...
OvseSpec should operate on **scopes**.
...
- OvseSpec reads only the relevant specs
- OvseSpec stores each artifact in the owning place
- If the work spans multiple owning places, OvseSpec creates a neutral coordination workspace
...
OvseSpec should model them explicitly as:
...
If there is no owner yet, the shared behavior can live temporarily as an initiative note during exploration, but OvseSpec should not promote it into a canonical spec until ownership is explicit.
...
Users should still recognize the existing OvseSpec workflow:
...
The new behavior is that OvseSpec may ask:
...
Instead, OvseSpec should create a neutral coordination workspace for the initiative.
...
OvseSpec may surface them to users and agents, but should not require them to resolve or validate across roots in the first version.

### 6. Shared contracts need explicit ownership

When the user describes behavior that spans multiple boundaries, OvseSpec should help them decide whether they are creating:

- a local spec
- a shared contract
- an initiative note that is not yet canonical

If the work is a real long-lived contract, OvseSpec should prompt for a canonical owner rather than burying it in whichever repo the user happened to be standing in.

### 7. Do not force "shared ownership" as implicit co-ownership

OvseSpec should help teams create **shared contract ownership**, not ambiguous co-ownership.
...
When a user starts in `web-client` and describes a checkout behavior spanning web, iOS, Android, and backend, OvseSpec should treat the current cwd as a clue about likely consumers, not as proof that `web-client` should own the shared contract.
...
A single OvseSpec root where canonical specs and changes can live.

In practice a project may be:

- a repo root with `ovsespec/`
- a monorepo root with one `ovsespec/`
- a nested owned scope if OvseSpec later supports nested roots
...
```text
~/work/ovsespec-workspaces/add-3ds/
  .ovsespec-workspace/
    workspace.yaml
    initiative.md
    links.yaml
...
```text
monorepo/
  apps/web/ovsespec.scope.yaml
  services/billing/ovsespec.scope.yaml
  packages/contracts/ovsespec.scope.yaml
```
...
OvseSpec needs to answer two separate questions:
...
OvseSpec discovers candidate scopes from:
...
OvseSpec resolves these to local paths using this order:

1. explicit path passed for this run
2. saved local OvseSpec registry
...
For team use, OvseSpec should separate:
...
OvseSpec should communicate this clearly:
...
OvseSpec should help teams choose the owner that can most credibly do all of the following:
...
When OvseSpec detects a likely shared contract, it should ask or infer from these questions:
...
OvseSpec should default to that existing owner.
...
OvseSpec should suggest that existing domain owner if there is no dedicated shared area.
...
OvseSpec should help create a new shared contract scope.
...
OvseSpec should allow the behavior to remain in initiative notes temporarily and explicitly mark it as non-canonical until ownership is chosen.

### What OvseSpec should suggest by default
...
When the user describes a likely cross-boundary behavior, OvseSpec should treat this as a first-class planning moment.
...
During `/ovsx:propose` or `/ovsx:explore`, OvseSpec should ask something like:
...
If the user chooses "create a new shared contract scope," OvseSpec should guide them.
...
- ovsespec/specs/contracts/checkout
- ovsespec/specs/shared/checkout
...
OvseSpec should avoid auto-creating a brand-new repo. It can scaffold the plan and record the decision, but repo creation may be organizationally sensitive and usually belongs outside the CLI.

### Temporary initiative-only mode

If ownership is unclear, OvseSpec should support:
...
4. Let OvseSpec determine whether this is:
   - local only
   - multi-scope in one root
   - cross-root coordination
5. OvseSpec creates artifacts in the right places
6. OvseSpec tells the user where to continue planning and where to implement
...
OvseSpec should support three agent capability levels.
...
- OvseSpec tells the user exactly which roots to attach
...
- User opens the coordination workspace
- OvseSpec writes agent-readable absolute paths into the workspace instructions
...
- OvseSpec guides the user into repo-local implementation sessions afterward
- `/ovsx:apply` runs in each repo separately

This must be treated as a first-class case, not a fallback afterthought.

---

## Current OvseSpec Flow To Preserve

Today the flow is:

1. User enters a repo
2. User runs `ovsespec init`
3. User opens an agent in that repo
4. User runs `/ovsx:explore` or `/ovsx:propose`
5. OvseSpec stores changes in `ovsespec/changes/<change>/`
6. `/ovsx:apply` implements within that repo

That should remain the exact feel for single-repo work.

The only extension is:

- if OvseSpec detects a cross-root initiative, it explicitly upgrades the workflow into a coordination workspace flow

---

## Journey 1: Single Repo, Standard OvseSpec Project
...
```text
Created ovsespec/changes/add-dark-mode/
Using scope: repo
...
Ready for implementation with /ovsx:apply
```
...
OvseSpec archives the change in this repo, as today.
...
- User runs from subdirectory: OvseSpec should either walk up or clearly tell them to run from repo root
...
```text
/ovsx:propose add-invoice-filtering
```

OvseSpec detects this is still one project root, but there are multiple candidate scopes.

If the user’s request clearly mentions one area, OvseSpec may infer:
...
1. create a single monorepo-local change under `ovsespec/changes/add-invoice-filtering/`
...
```text
/ovsx:propose add-3ds-checkout
```

OvseSpec detects likely affected scopes:
...
```text
platform/ovsespec/changes/add-3ds-checkout/
```
...
OvseSpec ignores unrelated specs by default.
...
This means OvseSpec cannot assume:
...
```text
apps/web/ovsespec.scope.yaml
apps/mobile/ovsespec.scope.yaml
services/billing/ovsespec.scope.yaml
packages/contracts/ovsespec.scope.yaml
```
...
```text
/ovsx:propose add-checkout-loading-state
```

OvseSpec detects this is limited to `apps/web`.
...
At this point OvseSpec must make a product decision.
...
If the user chooses the coordination flow, OvseSpec creates:
...
They already use OvseSpec there.
...
At this point OvseSpec should not silently create one cross-repo change inside `web-client`.
...
### Expected OvseSpec behavior

OvseSpec interrupts the default single-repo flow and says:
...
For cross-repo work, OvseSpec recommends creating a coordination workspace.
...
If the user agrees, OvseSpec creates the workspace.
...
```bash
ovsespec workspace create add-3ds --at ~/work/ovsespec-workspaces/add-3ds
```
...
OvseSpec now resolves local paths for:
...
OvseSpec then tells the user:

```text
Next step:
1. Open your coding agent in ~/work/ovsespec-workspaces/add-3ds
...
OvseSpec has generated workspace instructions at:
.ovsespec-workspace/agents/claude.md
```
...
Or OvseSpec may have already scaffolded the initiative and tell the agent to continue it.
...
- `contracts/ovsespec/changes/add-3ds-contract/`
- `billing-service/ovsespec/changes/add-3ds-billing/`
- `web-client/ovsespec/changes/add-3ds-web/`
- `ios-client/ovsespec/changes/add-3ds-ios/`
...
```bash
ovsespec initiative new add-3ds
```
...
OvseSpec resolves local clones and writes the workspace files.

### Agent setup

The user opens the coordination workspace in their agent.

OvseSpec-generated agent instructions contain:
...
OvseSpec should support that directly.
...
Each teammate keeps local path mappings outside the shared repo, for example in OvseSpec local config/data:
...
2. runs something like `ovsespec workspace doctor` or `ovsespec workspace sync`
...
OvseSpec generates agent instructions using:
...
OvseSpec should support the initiative being created in a shared coordination repo owned by the sponsoring or driving team.
...
When another team member opens the shared coordination workspace, OvseSpec should make this explicit:
...
OvseSpec should still allow planning.
...
OvseSpec should never blindly read all specs from all roots.
...
```text
Related references:
- github.com/Fission-AI/contracts: ovsespec/specs/checkout/spec.md
- github.com/Fission-AI/web-client: ovsespec/specs/checkout/web/spec.md
```
...
When a selected change involves a shared contract, OvseSpec should prefer this read order:
...
If a cross-boundary behavior has not yet been assigned a canonical owner, OvseSpec should store it as initiative-level draft material rather than pretending it is already a canonical spec.
...
## Journey 10: `/ovsx:apply` In Cross-Root Work
...
Next suggested step:
- open /Users/me/work/contracts and run /ovsx:apply add-3ds-contract
```
...
OvseSpec detects this likely requires a shared contract.
...
If the user selects an existing contracts repo, OvseSpec:
...
If the user keeps it initiative-only, OvseSpec:
...
OvseSpec checks linked repo changes:
...
4. run `ovsespec update`
5. open agent in monorepo root

During planning, OvseSpec asks for scope selection when needed.
...
1. each repo independently runs `ovsespec init`
...
When OvseSpec creates a coordination workspace, it should generate agent-facing files.
...
- OvseSpec recommends creating a coordination workspace
...
- `ovsespec workspace doctor` or equivalent relinks roots
...
If OvseSpec adopts these journeys, several design conclusions follow.

### 1. Multi-repo planning needs a first-class coordination workspace

Without this, OvseSpec has no honest answer to where the user should stand or where the cross-repo initiative should live.

### 2. Repo-local changes and canonical specs should stay in owners

Without this, OvseSpec creates duplicate or misleading sources of truth.

### 3. Large monorepos cannot be treated as always-simple single roots

OvseSpec must support both:
...
OvseSpec must separately handle:
...
Cross-root planning only works reliably if OvseSpec writes deterministic workspace context for the agent.

### 6. References should stay informational in v1

If OvseSpec validates cross-root references by default, it has effectively shipped a dependency graph system.
...
OvseSpec should help users choose among:
...
To support teams and orgs cleanly, OvseSpec should distinguish:
...
1. Should a coordination workspace always be user-visible on disk, or may it optionally live in global OvseSpec data directories?
...
3. Should OvseSpec support nested `ovsespec/` roots, or keep one root and model scopes separately?
