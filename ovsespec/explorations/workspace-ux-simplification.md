...
> OvseSpec should have one default path, one escalation path, and fewer explicit concepts shown to the user unless the system actually needs a decision from them.
...
2. Cross-root work feels like a workflow restart:
   ...
   - OvseSpec says this is multi-repo
...
- "OvseSpec figures out whether this stays local or needs to expand"
- "If it expands, it carries me forward instead of making me restart"
- "I only see advanced concepts when OvseSpec needs a real decision from me"
...
2. Run `/ovsx:explore` or `/ovsx:propose`
3. OvseSpec plans locally unless it has a strong reason not to
...
> This work spans multiple owned areas strongly enough that OvseSpec needs to upgrade it into a coordinated initiative.
...
- "OvseSpec is upgrading this into a coordinated initiative"
...
### Concepts OvseSpec should keep implicit until needed
...
### Concepts OvseSpec should only show when a real decision is needed
...
If OvseSpec realizes the work is no longer local, it should escalate in one motion.
...
If escalation is needed, OvseSpec should do as much as possible automatically:
...
OvseSpec can upgrade this into a coordinated initiative.

Suggested next step:
- create a coordination workspace at ~/work/ovsespec-workspaces/add-3ds
...
When OvseSpec has to ask questions, it should ask the smallest useful set.
...
Only ask if OvseSpec has strong evidence of a cross-boundary contract.
...
To keep the UX intuitive, OvseSpec should aggressively choose defaults.
...
OvseSpec should infer affected areas from:
...
If an existing shared contract owner already exists, OvseSpec should suggest it instead of asking an abstract ownership question.
...
OvseSpec can start with a local coordination workspace and later offer:
...
2. OvseSpec upgrades to a coordinated initiative if needed
3. When the work becomes collaborative, OvseSpec offers to make it team-shared
...
OvseSpec should avoid asking users to classify work in abstract ways if it can infer a reasonable default.
...
By default, OvseSpec should feel like:
...
- progressive disclosure: only show advanced concepts when OvseSpec needs a real decision

If OvseSpec does this well, the same system can feel intuitive for:
