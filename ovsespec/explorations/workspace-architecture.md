...
```
ovsespec profile set core|extended
ovsespec profile install <workflow>
ovsespec profile uninstall <workflow>
ovsespec profile list
ovsespec profile show
ovsespec config set delivery skills|commands|both
ovsespec config get delivery
...
**After (simplified):**
```
ovsespec config profile          # interactive picker (delivery + workflows)
ovsespec config profile core     # preset shortcut
ovsespec config profile extended # preset shortcut
```
...
- **Global** = user's defaults (`~/.config/ovsespec/`)
- **Project** = repo-level config (`.ovsespec/` or committed to repo)
...
Keep it simple:
1. Global profile as default for `ovsespec init`
2. `ovsespec init` applies current profile to project
...
### Current OvseSpec Architecture

OvseSpec currently assumes:
- One `ovsespec/` per repo, always at root
...
## Part 4: Three Models for OvseSpec
...
├── services/
│   └── billing/
│       └── ovsespec/specs/billing/spec.md
├── clients/
│   ├── web/
│   │   └── ovsespec/specs/checkout/spec.md
│   ├── ios/
│   │   └── ovsespec/specs/checkout/spec.md
│   └── android/
│       └── ovsespec/specs/checkout/spec.md
└── ovsespec/           # Root-level for cross-cutting
    ├── specs/
...
- Works for multi-repo too (each repo has its own `ovsespec/`)

**Cons:**
- Cross-cutting specs are awkward (where do they go?)
- Changes that span multiple `ovsespec/` directories = ???
- Need a "workspace" concept to aggregate
- Multiple `ovsespec/` roots to manage

### Model D: Hybrid (Model B Inside Each Project + Model C Across Projects)

Use one `ovsespec/` root per project, but allow nested specs within that root for clear ownership and shared contracts.
...
~/work/
├── billing-service/
│   └── ovsespec/specs/billing/
├── web-client/
│   └── ovsespec/specs/checkout/
├── ios-client/
│   └── ovsespec/specs/checkout/
...
2. **Where do cross-repo changes live?**
   ...
   - In `~/.config/ovsespec/workspaces/my-platform/changes/`?
...
> **A workspace is a collection of OvseSpec roots that can be operated on together.**

```yaml
# ~/.config/ovsespec/workspaces.yaml (or similar)
workspaces:
  my-platform:
    roots:
...
- `ovsespec/specs/checkout/spec.md` is the shared kernel
- Platform specs nest under it

**Option B: Dedicated area**
- `ovsespec/specs/_shared/checkout-contract/spec.md`
- Or `ovsespec/specs/_contracts/checkout/spec.md`
...
| Concept | Definition |
|---------|------------|
| **Project** | Single OvseSpec root (one `ovsespec/` directory) |
...
### 4. Does OvseSpec need to understand dependencies?

If `checkout-web` depends on `checkout-contract`:
- Should OvseSpec know this relationship?
...
**Ambitious:**
> OvseSpec automatically understands your repo structure, detects cross-cutting specs, and helps you create changes that flow to the right places.

**Simpler:**
> You organize specs however you want. OvseSpec just works.
...
### What is a spec in OvseSpec?

For OvseSpec, a spec should be treated as a **verifiable behavior contract at a boundary**:
...
OvseSpec is often agent-authored from human exploration. To make that reliable:
...
2. `ovsespec/specs/ovsespec-conventions/spec.md` for normative spec conventions
3. `ovsespec/specs/docs-agent-instructions/spec.md` for agent-instruction authoring rules

---

## Part 10: Design Decisions (April 2026)

After evaluating the models above against real multi-repo use cases (see [#725](https://github.com/Fission-AI/OvseSpec/issues/725)), we converged on the following design direction.
...
- **Each repo keeps its own canonical `ovsespec/`** — no change to the fundamental storage model.
...
For larger multi-team work, repo-centric planning is the wrong primary abstraction. Teams and repos are many-to-many facets over the same work. OvseSpec should treat the **initiative / plan** as the first-class planning object, then link repo-local changes to it.
...
```yaml
# billing-service/ovsespec/changes/add-3ds/.ovsespec.yaml
schema: spec-driven
created: 2026-04-12
initiative: add-3ds
links:
...
2. **Local OvseSpec repo registry** — a persistent mapping in `~/.config/ovsespec/` or `~/.local/share/ovsespec/` (see `src/core/global-config.ts`).
...
```yaml
# web-client/ovsespec/specs/checkout/spec.md frontmatter
references:
...
This avoids accidentally committing OvseSpec to a full dependency graph system before the use cases justify it.
...
|---------|----------|------------|
| **Spec organization** | Nested specs inside one `ovsespec/` (Model B) | Each repo has its own `ovsespec/` |
...
|----------|--------|-------|
| Profile UX | Decided | `ovsespec config profile` with presets |
