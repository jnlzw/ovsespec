...
## Purpose
Define `ovsespec schema init` behavior for creating project-local schema skeletons in interactive and non-interactive modes.

## Requirements
### Requirement: Schema init command creates project-local schema
The CLI SHALL provide an `ovsespec schema init <name>` command that creates a new schema directory under `ovsespec/schemas/<name>/` with a valid `schema.yaml` file and default template files.

#### Scenario: Create schema with valid name
- **WHEN** user runs `ovsespec schema init my-workflow`
- **THEN** system creates directory `ovsespec/schemas/my-workflow/`
...
- **WHEN** user runs `ovsespec schema init "My Workflow"` (contains space)
...
- **WHEN** user runs `ovsespec schema init existing-schema` and `ovsespec/schemas/existing-schema/` already exists
...
- **WHEN** user runs `ovsespec schema init my-workflow` in an interactive terminal
...
- **WHEN** user runs `ovsespec schema init my-workflow --description "My workflow" --artifacts proposal,tasks`
...
- **WHEN** user runs `ovsespec schema init my-workflow` in interactive mode
- **AND** user confirms setting as default
- **THEN** system updates `ovsespec/config.yaml` with `defaultSchema: my-workflow`

#### Scenario: Set as default via flag
- **WHEN** user runs `ovsespec schema init my-workflow --default`
- **THEN** system creates schema and updates `ovsespec/config.yaml` with `defaultSchema: my-workflow`

#### Scenario: Skip setting default
- **WHEN** user runs `ovsespec schema init my-workflow --no-default`
- **THEN** system creates schema without modifying `ovsespec/config.yaml`
...
- **WHEN** user runs `ovsespec schema init my-workflow --json --description "Test" --artifacts proposal`
...
- **WHEN** user runs `ovsespec schema init "invalid name" --json`
