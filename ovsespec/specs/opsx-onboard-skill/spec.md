# ovsx-onboard-skill Specification

## Purpose
Define `/ovsx:onboard` behavior for guiding users through an end-to-end OvseSpec workflow on their real codebase.

## Requirements
### Requirement: OVSX Onboard Skill

The system SHALL provide an `/ovsx:onboard` skill that guides users through their first complete OvseSpec workflow cycle with narration and real codebase work.

#### Scenario: Skill invocation

- **WHEN** user invokes `/ovsx:onboard`
- **THEN** agent checks if OvseSpec is initialized
- **AND** if not initialized, prompts user to run `ovsespec init` first
...
- **THEN** agent briefly demonstrates `/ovsx:explore` by investigating relevant code
...
- **THEN** agent runs `ovsespec new change "<name>"` with derived kebab-case name
...
- **AND** provides command reference table (/ovsx:explore, /ovsx:new, /ovsx:ff, /ovsx:continue, /ovsx:apply, /ovsx:verify, /ovsx:archive)
- **AND** suggests next actions (try /ovsx:new or /ovsx:ff on something)
...
- **AND** explains how to continue later with `/ovsx:continue <name>`
...
- **AND** exits gracefully with encouragement to try `/ovsx:new`
