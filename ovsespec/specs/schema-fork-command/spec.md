...
## Purpose
Define `ovsespec schema fork` behavior for cloning existing schemas into project-local schemas with safe overwrite controls.

## Requirements
### Requirement: Schema fork copies existing schema
The CLI SHALL provide an `ovsespec schema fork <source> [name]` command that copies an existing schema to the project's `ovsespec/schemas/` directory.

#### Scenario: Fork with explicit name
- **WHEN** user runs `ovsespec schema fork spec-driven my-custom`
- **THEN** system locates `spec-driven` schema using resolution order (project → user → package)
- **AND** copies all files to `ovsespec/schemas/my-custom/`
...
- **WHEN** user runs `ovsespec schema fork spec-driven` without specifying a name
- **THEN** system copies to `ovsespec/schemas/spec-driven-custom/`
- **AND** updates `name` field in `schema.yaml` to `spec-driven-custom`

#### Scenario: Source schema not found
- **WHEN** user runs `ovsespec schema fork nonexistent`
...
- **WHEN** user runs `ovsespec schema fork spec-driven my-custom` and `ovsespec/schemas/my-custom/` exists
...
- **WHEN** user runs `ovsespec schema fork spec-driven my-custom --force` and destination exists
...
- **WHEN** user runs `ovsespec schema fork spec-driven my-custom` in interactive mode and destination exists
...
- **WHEN** user runs `ovsespec schema fork spec-driven my-custom --json`
...
- **WHEN** user runs `ovsespec schema fork spec-driven --json`
