...
## Purpose
Define detection and cleanup behavior for legacy OvseSpec artifacts during initialization and update workflows.

## Requirements
### Requirement: Legacy artifact detection

The system SHALL detect legacy OvseSpec artifacts from previous init versions.

#### Scenario: Detecting legacy config files

- **WHEN** running `ovsespec init` on an existing project
- **THEN** the system SHALL check for config files with OvseSpec markers:
  - `CLAUDE.md`
  - `.cursorrules`
  - `.windsurfrules`
  - `.clinerules`
...
- **THEN** the system SHALL check for old slash command directories:
  ...
  - `.cursor/commands/ovsespec/` (note: old format used `ovsespec-*.md` in commands root)
  - `.windsurf/workflows/ovsespec-*.md`
  - And equivalent directories for all tools in the legacy SlashCommandRegistry

#### Scenario: Detecting legacy OvseSpec structure files

- **WHEN** running `ovsespec init` on an existing project
- **THEN** the system SHALL check for:
  - `ovsespec/AGENTS.md`
  - `ovsespec/project.md` (for migration messaging only, not deleted)
  - Root `AGENTS.md` with OvseSpec markers
...
The system SHALL preserve user content when removing OvseSpec markers from config files.

#### Scenario: Config file with only OvseSpec content

- **WHEN** a config file contains only OvseSpec marker block (whitespace outside is acceptable)
- **THEN** the system SHALL remove the OvseSpec marker block
...
- **WHEN** a config file contains content outside OvseSpec markers
- **THEN** the system SHALL remove only the `<!-- OVSESPEC:START -->` to `<!-- OVSESPEC:END -->` block
...
- **WHEN** root `AGENTS.md` contains OvseSpec markers AND other content
- **THEN** the system SHALL remove only the OvseSpec marker block
...
- **WHEN** a legacy slash command directory exists (e.g., `.claude/commands/ovsespec/`)
...
- **WHEN** `ovsespec/AGENTS.md` exists
- **THEN** the system SHALL delete the file
- **AND** NOT delete the `ovsespec/` directory itself
...
- **THEN** users can migrate manually or use `/ovsx:explore` to get AI assistance
...
- **THEN** the system SHALL display a summary section:
  ...
  Cleaned up legacy files:
    ✓ Removed OvseSpec markers from CLAUDE.md
    ✓ Removed .claude/commands/ovsespec/ (replaced by /ovsx:*)
    ✓ Removed ovsespec/AGENTS.md (no longer needed)
  ```
- **AND IF** `ovsespec/project.md` exists
