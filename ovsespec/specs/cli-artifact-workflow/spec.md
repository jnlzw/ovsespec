...
- **WHEN** user runs `ovsespec status --change <id>`
...
- **WHEN** user runs `ovsespec status --change <id> --json`
...
- **WHEN** user runs `ovsespec status --change <id>` on a change with no artifacts
...
- **WHEN** user runs `ovsespec status` without `--change`
...
- **WHEN** user runs `ovsespec status --change unknown-id`
- **AND** directory `ovsespec/changes/unknown-id/` does not exist
- **THEN** the system displays an error listing all available change directories

### Requirement: Next Artifact Discovery

The workflow SHALL use `ovsespec status` output to determine what can be created next, rather than a separate next-command surface.

#### Scenario: Discover next artifacts from status output

- **WHEN** a user needs to know which artifact to create next
- **THEN** `ovsespec status --change <id>` identifies ready artifacts with `[ ]`
...
- **WHEN** user runs `ovsespec instructions <artifact> --change <id>`
...
- **WHEN** user runs `ovsespec instructions <artifact> --change <id> --json`
- **THEN** the system outputs JSON matching ArtifactInstructions interface

#### Scenario: Unknown artifact

- **WHEN** user runs `ovsespec instructions unknown-artifact --change <id>`
...
- **WHEN** user runs `ovsespec instructions proposal --change <id>` on a scaffolded change
...
- **WHEN** user runs `ovsespec templates`
...
- **THEN** the CLI SHALL support `ovsespec workspace ls`
- **AND** it SHALL behave the same as `ovsespec workspace list`
...
- **THEN** the CLI SHALL use `ovsespec workspace setup`
- **AND** it SHALL not expose `ovsespec workspace create` as the public creation path
...
The system SHALL generate schema-aware apply instructions via `ovsespec instructions apply`.

#### Scenario: Generate apply instructions

- **WHEN** user runs `ovsespec instructions apply --change <id>`
...
- **WHEN** user runs `ovsespec instructions apply --change <id> --json`
- **THEN** the system outputs JSON with:
...
- **WHEN** user runs `ovsespec artifact-experimental-setup --tool cursor`
...
- **WHEN** user runs `ovsespec artifact-experimental-setup` without `--tool`
...
- **WHEN** user runs `ovsespec artifact-experimental-setup --tool unknown-tool`
