...
## Purpose
Define `ovsespec feedback` behavior for creating GitHub issues safely via `gh`, with a manual fallback when automation is unavailable.

## Requirements
### Requirement: Feedback command

The system SHALL provide an `ovsespec feedback` command that creates a GitHub Issue in the ovsespec repository using the `gh` CLI. The system SHALL use `execFileSync` with argument arrays to prevent shell injection vulnerabilities.

#### Scenario: Simple feedback submission

- **WHEN** user executes `ovsespec feedback "Great tool!"`
- **THEN** the system executes `gh issue create` with title "Feedback: Great tool!"
- **AND** the issue is created in the ovsespec repository
...
- **WHEN** user executes `ovsespec feedback "Title here" --body "Detailed description..."`
- **THEN** the system creates a GitHub Issue with the specified title
- **AND** the issue body contains the detailed description
- **AND** the issue body includes metadata (OvseSpec version, platform, timestamp)
...
- **WHEN** user runs `ovsespec feedback "message"`
...
- **THEN** the issue body includes:
  - OvseSpec CLI version
  - Platform (darwin, linux, win32)
  - Submission timestamp
  - Separator line: "---\nSubmitted via OvseSpec CLI"
...
- **WHEN** user has disabled telemetry via `OVSESPEC_TELEMETRY=0`
- **AND** user runs `ovsespec feedback "message"`
...
- **AND** submits via `ovsespec feedback` command on user confirmation
...
- **WHEN** user types `ovsespec fee<TAB>`
- **THEN** the shell completes to `ovsespec feedback`

#### Scenario: Flag completion

- **WHEN** user types `ovsespec feedback "msg" --<TAB>`
