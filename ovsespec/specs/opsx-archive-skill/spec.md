# OVSX Archive Skill Spec

## Purpose

Define the expected behavior for the `/ovsx:archive` skill, including readiness checks, spec sync prompting, archive execution, and user-facing output.

## Requirements

### Requirement: OVSX Archive Skill

The system SHALL provide an `/ovsx:archive` skill that archives completed changes in the experimental workflow.

#### Scenario: Archive a change with all artifacts complete

- **WHEN** agent executes `/ovsx:archive` with a change name
- **AND** all artifacts in the schema are complete
- **AND** all tasks are complete
- **THEN** the agent moves the change to `ovsespec/changes/archive/YYYY-MM-DD-<name>/`
- **AND** displays success message with archived location

#### Scenario: Change selection prompt

- **WHEN** agent executes `/ovsx:archive` without specifying a change
...
- **AND** if user confirms, execute `/ovsx:sync` logic
...
- **AND** preserve `.ovsespec.yaml` file in archived change
...
- **THEN** display summary:
  - Specs synced (from `/ovsx:sync` output)
