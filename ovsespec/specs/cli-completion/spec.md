...
## Purpose
Provide shell completion scripts for the OvseSpec CLI, enabling tab-completion for commands, flags, and dynamic values (change IDs, spec IDs) across multiple shells. Supports Zsh, Bash, Fish, and PowerShell.
## Requirements
...
- **WHEN** user executes `ovsespec completion --help`
...
- **WHEN** user executes `ovsespec completion generate zsh`
...
- **WHEN** user executes `ovsespec completion generate bash`
...
- **AND** support dynamic completion for change and spec IDs via `ovsespec __complete`

#### Scenario: Generating Fish completion

- **WHEN** user executes `ovsespec completion generate fish`
- **THEN** output a complete Fish completion script to stdout
- **AND** use `complete -c ovsespec` with conditions
...
- **WHEN** user executes `ovsespec completion generate powershell`
- **THEN** output a complete PowerShell completion script to stdout
- **AND** use `Register-ArgumentCompleter -CommandName ovsespec`
...
- **THEN** discover active changes from `ovsespec/changes/` directory
- **AND** exclude archived changes in `ovsespec/changes/archive/`
- **AND** return change IDs as completion suggestions
- **AND** only provide suggestions when inside an OvseSpec-enabled project

#### Scenario: Completing spec IDs

- **WHEN** completing arguments for commands that accept spec names (show, validate)
- **THEN** discover specs from `ovsespec/specs/` directory
...
- **WHEN** user requests completions outside an OvseSpec project
...
- **WHEN** user executes `ovsespec completion install zsh`
...
- **AND** write completion script to `~/.oh-my-zsh/custom/completions/_ovsespec`
...
- **WHEN** user executes `ovsespec completion install zsh` and Oh My Zsh is not detected
- **THEN** create completions directory at `~/.zsh/completions/` if it doesn't exist
- **AND** write completion script to `~/.zsh/completions/_ovsespec`
...
- **WHEN** user executes `ovsespec completion install bash`
- **THEN** detect if bash-completion is installed by checking for `/usr/share/bash-completion` or `/etc/bash_completion.d`
- **AND** if bash-completion is available, write to `/etc/bash_completion.d/ovsespec` (with sudo) or `~/.local/share/bash-completion/completions/ovsespec`
- **AND** if bash-completion is not available, write to `~/.bash_completion.d/ovsespec` and source it from `~/.bashrc`
...
- **WHEN** user executes `ovsespec completion install fish`
- **THEN** create Fish completions directory at `~/.config/fish/completions/` if it doesn't exist
- **AND** write completion script to `~/.config/fish/completions/ovsespec.fish`
...
- **WHEN** user executes `ovsespec completion install powershell`
...
- **WHEN** user executes `ovsespec completion install` without specifying a shell
...
- **WHEN** user executes `ovsespec completion uninstall zsh`
...
- **AND** if user confirms, remove `~/.oh-my-zsh/custom/completions/_ovsespec` if Oh My Zsh is detected
- **AND** remove `~/.zsh/completions/_ovsespec` if standard Zsh setup is detected
- **AND** remove fpath modifications from `~/.zshrc` using marker-based removal
- **AND** display success message

#### Scenario: Uninstalling Bash completion

- **WHEN** user executes `ovsespec completion uninstall bash`
...
- **WHEN** user executes `ovsespec completion uninstall fish`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
- **AND** if user confirms, remove `~/.config/fish/completions/ovsespec.fish`
- **AND** display success message (no config file modification needed)

#### Scenario: Uninstalling PowerShell completion

- **WHEN** user executes `ovsespec completion uninstall powershell`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
...
- **WHEN** user executes `ovsespec completion uninstall` without specifying a shell
...
- **AND** implement methods:
  ...
  - `isOvseSpecProject(): boolean` - Checks if current directory is OvseSpec-enabled
...
- **WHEN** `ovsespec completion install` cannot detect current shell
- **THEN** display error: "Could not auto-detect shell. Please specify shell explicitly."
- **AND** display usage hint: "Usage: ovsespec completion <operation> [shell]"
- **AND** exit with code 1
...
- **AND** allow redirection to files: `ovsespec completion generate zsh > /path/to/_ovsespec`
