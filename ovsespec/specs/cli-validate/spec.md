...
## Purpose
Define `ovsespec validate` behavior for validating changes and specs with actionable remediation guidance and structured output.
...
- **THEN** show error "No deltas found" with guidance:
  ...
  - Remind authors that files must live under `ovsespec/changes/{id}/specs/<capability>/spec.md`
  - Include an explicit note: "Spec delta files cannot start with titles before the operation headers"
  - Suggest running `ovsespec change show {id} --json --deltas-only` for debugging
...
- **THEN** include expected header names and a minimal skeleton:
  ...
  - Mention the quick-reference section in `ovsespec/AGENTS.md` as the authoritative template
...
- **THEN** emit an error explaining that `### Requirement:` lines must be followed by narrative text before any `#### Scenario:` headers
  ...
  - Reference the pre-validation checklist in `ovsespec/AGENTS.md`
...
- Source file path (`ovsespec/changes/{id}/proposal.md`, `.../specs/{cap}/spec.md`)
...
- **THEN** print "Next steps" with 2-3 targeted bullets and suggest `ovsespec change show <id> --json --deltas-only`
...
- **WHEN** executing `ovsespec validate` without arguments
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `OVSE_SPEC_INTERACTIVE=0`
- **WHEN** executing `ovsespec validate` without arguments
...
- **WHEN** executing `ovsespec validate <item-name>`
...
- **WHEN** executing `ovsespec validate --all`
- **THEN** validate all changes in ovsespec/changes/ (excluding archive)
- **AND** validate all specs in ovsespec/specs/
...
- **THEN** include all change proposals under `ovsespec/changes/`
- **AND** exclude the `ovsespec/changes/archive/` directory

- **WHEN** validating with `--specs`
- **THEN** include all specs that have a `spec.md` under `ovsespec/specs/<id>/spec.md`

#### Scenario: Validate all changes

- **WHEN** executing `ovsespec validate --changes`
- **THEN** validate all changes in ovsespec/changes/ (excluding archive)
...
- **WHEN** executing `ovsespec validate --specs`
- **THEN** validate all specs in ovsespec/specs/
...
- **WHEN** executing `ovsespec validate --all --strict`
...
- **WHEN** executing `ovsespec validate --all --json`
...
- **WHEN** executing `ovsespec validate --all --json` (or `--changes` / `--specs`)
...
- **AND** suggest passing `--type change` or `--type spec`, or using `ovsespec change validate` / `ovsespec spec validate`
...
- **WHEN** executing `ovsespec validate --type change <item>`
- **THEN** treat `<item>` as a change ID and validate it (skipping auto-detection)

- **WHEN** executing `ovsespec validate --type spec <item>`
...
- The CLI SHALL respect `OVSE_SPEC_INTERACTIVE=0` to disable prompts globally.
...
- **WHEN** `ovsespec validate` is executed with `--no-interactive` or with environment `OVSE_SPEC_INTERACTIVE=0`
...
- **WHEN** running `ovsespec validate <change-id>`
