...
## Purpose
Define `ovsespec change` command behavior for showing, listing, and validating change proposals and deltas.
...
- **WHEN** executing `ovsespec change show update-error --json`
...
- **WHEN** executing `ovsespec change list`
- **THEN** scan the ovsespec/changes directory
- **AND** return list of all pending changes
- **AND** support JSON output with `--json` flag

#### Scenario: Show only requirement changes

- **WHEN** executing `ovsespec change show update-error --requirements-only`
...
- **WHEN** executing `ovsespec change validate update-error`
...
- **WHEN** executing `ovsespec list`
- **THEN** display current list of changes (existing behavior)
- **AND** show deprecation notice: "Note: 'ovsespec list' is deprecated. Use 'ovsespec change list' instead."

#### Scenario: Legacy list with --all flag

- **WHEN** executing `ovsespec list --all`
...
- **WHEN** executing `ovsespec change show` without arguments
...
- **WHEN** executing `ovsespec change show` without a change name
...
- **WHEN** executing `ovsespec change validate` without arguments
- **THEN** display an interactive list of available changes
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `OVSE_SPEC_INTERACTIVE=0`
- **WHEN** executing `ovsespec change validate` without a change name
- **THEN** do not prompt interactively
- **AND** print the existing hint including available change IDs
- **AND** set `process.exitCode = 1`
