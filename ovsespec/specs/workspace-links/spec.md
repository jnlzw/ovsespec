...
and JSON-output behavior for managing OvseSpec workspaces across repos and
folders.

## Requirements
### Requirement: Guided Workspace Setup
OvseSpec SHALL provide a guided setup flow for users starting workspace planning.

#### Scenario: Creating a workspace through setup
- **WHEN** a user runs `ovsespec workspace setup`
- **THEN** OvseSpec SHALL guide the user through creating an OvseSpec workspace
...
- **THEN** OvseSpec SHALL ask for the workspace name before asking for repos or folders
...
- **THEN** OvseSpec SHALL explain that workspace names must be kebab-case
...
- **THEN** OvseSpec SHALL infer the link name from the folder basename
...
- **THEN** OvseSpec SHALL show the conflicting link name and the existing path for that link
...
- **THEN** OvseSpec SHALL let the user add another repo or folder with a simple repeated prompt
...
- **THEN** OvseSpec SHALL verify that the path resolves to an existing folder
...
- **THEN** OvseSpec SHALL treat the full value as the path
...
- **WHEN** `ovsespec workspace setup --no-interactive` receives a workspace name and at least one valid link
- **THEN** OvseSpec SHALL create the workspace without prompts
- **AND** it SHALL support repeated `--link` values

#### Scenario: Non-interactive setup duplicate link names
- **WHEN** `ovsespec workspace setup --no-interactive` receives two links with the same inferred or explicit name
- **THEN** OvseSpec SHALL fail with a clear duplicate link-name error
...
- **WHEN** `ovsespec workspace setup --no-interactive` is missing a workspace name or link
- **THEN** OvseSpec SHALL fail with a clear message
- **AND** it SHALL explain which flags are required

#### Scenario: Finishing setup
- **WHEN** setup finishes
- **THEN** OvseSpec SHALL show the workspace location, planning path, and linked repos or folders
...
- **THEN** OvseSpec SHALL record it in the local workspace registry
...
- **THEN** OvseSpec SHALL explain that the workspace already exists
- **AND** it SHALL not overwrite the existing workspace

### Requirement: Workspace Discovery
OvseSpec SHALL let users see the OvseSpec-managed workspaces available on the current machine.

#### Scenario: Listing workspaces
- **WHEN** a user runs `ovsespec workspace list`
- **THEN** OvseSpec SHALL list known managed workspaces
...
- **WHEN** a user runs `ovsespec workspace ls`
- **THEN** OvseSpec SHALL behave the same as `ovsespec workspace list`

#### Scenario: Listing when no workspaces exist
- **WHEN** a user runs `ovsespec workspace list`
- **AND** no managed workspaces exist
- **THEN** OvseSpec SHALL say that no workspaces were found
...
- **THEN** OvseSpec SHALL use the current workspace
...
- **THEN** OvseSpec SHALL record the workspace name and location in the local workspace registry
...
- **THEN** OvseSpec SHALL report the non-fatal registry warning
...
- **THEN** OvseSpec SHALL show a workspace picker
...
- **THEN** OvseSpec SHALL fail without showing a picker
...
- **THEN** OvseSpec SHALL fail without showing a picker regardless of interactive mode
- **AND** it SHALL print `No known OvseSpec workspaces. Run 'ovsespec workspace setup' first.`
...
OvseSpec SHALL let users link existing repos or folders to a workspace before creating a change.

#### Scenario: Linking with an inferred name
- **WHEN** a user runs `ovsespec workspace link <path>`
- **THEN** OvseSpec SHALL infer the link name from the folder basename
- **AND** it SHALL store the verified absolute local path as machine-local state

#### Scenario: Linking with an explicit name
- **WHEN** a user runs `ovsespec workspace link <name> <path>`
- **THEN** OvseSpec SHALL use the explicit link name for planning
- **AND** it SHALL store the verified absolute local path as machine-local state
...
- **AND** OvseSpec SHALL reject missing paths with a clear message

#### Scenario: Resolving linked paths before storage
- **WHEN** a user links a repo or folder path
- **THEN** OvseSpec SHALL store the verified absolute path for the current runtime
- **AND** relative inputs SHALL be resolved against the command's current working directory
- **AND** OvseSpec SHALL not translate paths between native Windows, WSL2, and Unix runtimes

#### Scenario: Linking a monorepo folder
- **WHEN** a user links a package, service, app, or directory inside a monorepo
- **THEN** OvseSpec SHALL store it as a workspace link
- **AND** it SHALL not require that folder to have its own repo-local `ovsespec/` directory

#### Scenario: Linking without repo-local OvseSpec
- **WHEN** a user links a path that does not contain repo-local OvseSpec state
- **THEN** OvseSpec SHALL keep that repo or folder available for workspace planning
- **AND** it SHALL not treat missing repo-local OvseSpec state as a link failure

#### Scenario: Link records only
- **WHEN** a user links a repo or folder
- **THEN** OvseSpec SHALL record workspace state and local path state
...
- **WHEN** a user runs `ovsespec workspace link`
- **THEN** OvseSpec SHALL fail with status code `workspace_local_state_invalid`
...
- **THEN** OvseSpec SHALL explain that the link name is already in use by another link
...
OvseSpec SHALL let users update existing link paths without recreating the workspace.

#### Scenario: Updating a local path
- **GIVEN** a workspace has a link
- **WHEN** a user runs `ovsespec workspace relink <name> <path>`
- **THEN** OvseSpec SHALL keep the stable link name
...
- **GIVEN** the workspace machine-local state file exists but cannot be parsed or validated
- **WHEN** a user runs `ovsespec workspace relink`
- **THEN** OvseSpec SHALL fail with status code `workspace_local_state_invalid`
...
- **THEN** OvseSpec SHALL explain that the link name is unknown
...
- **THEN** OvseSpec SHALL not ask for owner or handoff metadata
- **AND** link maintenance SHALL focus on names and local paths

### Requirement: Workspace Health Check
OvseSpec SHALL explain what the current machine can resolve for a workspace.

#### Scenario: Doctor checks one selected workspace
- **WHEN** a user runs `ovsespec workspace doctor`
- **THEN** OvseSpec SHALL inspect one selected workspace
- **AND** it SHALL not scan every known workspace in the local registry by default

#### Scenario: Doctor infers the current workspace
- **GIVEN** the command runs from a workspace folder or subdirectory
- **WHEN** the user runs `ovsespec workspace doctor` without `--workspace <name>`
- **THEN** OvseSpec SHALL inspect the current workspace

#### Scenario: Checking a healthy workspace
- **WHEN** a user runs `ovsespec workspace doctor`
- **THEN** OvseSpec SHALL show the workspace location and workspace planning path
...
- **WHEN** a user runs `ovsespec workspace doctor`
- **THEN** OvseSpec SHALL report a selected-workspace status error
...
- **THEN** doctor SHALL report `repo_specs_path` when repo-local `ovsespec/specs` exists
...
- **THEN** OvseSpec SHALL report status code `workspace_local_state_invalid`
...
OvseSpec SHALL provide JSON output for direct workspace setup commands.

#### Scenario: Requesting JSON output
- **WHEN** a user passes `--json` to direct workspace setup commands
- **THEN** OvseSpec SHALL print machine-readable output
...
- **WHEN** a user runs `ovsespec workspace setup --json` without `--no-interactive`
- **THEN** OvseSpec SHALL fail clearly
...
- **THEN** OvseSpec SHALL avoid interactive prompts
