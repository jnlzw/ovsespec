...
## Purpose
Define top-level `ovsespec show` behavior for interactive and direct display of change and spec content.
...
- **WHEN** executing `ovsespec show` without arguments
...
- **WHEN** executing `ovsespec show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `ovsespec show <item>` or `ovsespec change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `ovsespec show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `ovsespec change show`/`ovsespec spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `ovsespec show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `ovsespec show --type spec <item>`
...
- **WHEN** executing `ovsespec show <item> --json`
...
- **WHEN** showing a change with `ovsespec show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `ovsespec show <spec-id> --requirements`
