...
The `ovsespec view` command provides a comprehensive dashboard view of the OvseSpec project state, displaying specifications, changes, and progress metrics in a unified, visually appealing format to help developers quickly understand project status.
## Requirements
...
- **WHEN** user runs `ovsespec view`
- **THEN** system displays a formatted dashboard with sections for summary, active changes, completed changes, and specifications

#### Scenario: No OvseSpec directory

- **WHEN** user runs `ovsespec view` in a directory without OvseSpec
- **THEN** system displays error message "✗ No ovsespec directory found"
