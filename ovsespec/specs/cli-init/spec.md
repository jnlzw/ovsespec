...
The `ovsespec init` command SHALL create a complete OvseSpec directory structure in any project, enabling immediate adoption of OvseSpec conventions with support for multiple AI coding assistants.
## Requirements
...
- **AND** display progress with ora spinners:
  - Show spinner: "⠋ Creating OvseSpec structure..."
  - Then success: "✔ OvseSpec structure created"
...
- **WHEN** the `ovsespec/` directory already exists
- **THEN** inform the user that OvseSpec is already initialized, skip recreating the base structure, and enter an extend mode
...
- **THEN** return appropriate exit code:
  - 0: Success
  - 1: General error (including when OvseSpec directory already exists)
  - 2: Insufficient permissions (reserved for future use)
  - 3: User cancelled operation (reserved for future use)

### Requirement: Additional AI Tool Initialization
`ovsespec init` SHALL allow users to add configuration files for new AI coding assistants after the initial setup.

#### Scenario: Configuring an extra tool after initial setup
- **GIVEN** an `ovsespec/` directory already exists and at least one AI tool file is present
- **WHEN** the user runs `ovsespec init` and selects a different supported AI tool
- **THEN** generate that tool's configuration files with OvseSpec markers the same way as during first-time initialization
...
`ovsespec init` SHALL summarize tool actions when initialization or extend mode completes.
...
`ovsespec init` SHALL treat extend mode without new native tool selections as a successful refresh.

#### Scenario: Allowing empty extend runs
- **WHEN** OvseSpec is already initialized and the user selects no additional natively supported tools
- **THEN** complete successfully without requiring additional tool setup
- **AND** preserve the existing OvseSpec structure and config files
...
- **THEN** create only the ovsespec directory structure
...
- **WHEN** user runs `ovsespec init` without `--tools`
- **THEN** fail with exit code 1
...
The command SHALL generate ovsx slash commands only for selected tools that have a registered command adapter, while keeping adapterless tools valid for skill generation.
...
- **THEN** OvseSpec SHALL treat it as a supported tool with `skillsDir: '.kimi'`
...
Manual creation of OvseSpec structure is error-prone and creates adoption friction. A standardized init command ensures:
