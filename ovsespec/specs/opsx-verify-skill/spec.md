# ovsx-verify-skill Specification

## Purpose
Define `/ovsx:verify` behavior for assessing implementation completeness, correctness, and coherence against change artifacts.

## Requirements
### Requirement: Verify Skill Invocation
The system SHALL provide an `/ovsx:verify` skill that validates implementation against change artifacts.

#### Scenario: Verify with change name provided
- **WHEN** agent executes `/ovsx:verify <change-name>`
...
- **WHEN** agent executes `/ovsx:verify` without a change name
...
- **AND** suggests running `/ovsx:continue` to create tasks
...
- **WHEN** verifying completeness
- **AND** delta specs exist in `ovsespec/changes/<name>/specs/`
