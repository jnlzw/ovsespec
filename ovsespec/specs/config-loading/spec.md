...
## Purpose
Define how `ovsespec/config.yaml` is discovered, parsed, validated, and exposed to callers with safe fallbacks.

## Requirements
### Requirement: Load project config from ovsespec/config.yaml

The system SHALL read and parse the project configuration file located at `ovsespec/config.yaml` relative to the project root.

#### Scenario: Valid config file exists
- **WHEN** `ovsespec/config.yaml` exists with valid YAML content
- **THEN** system parses the file and returns a ProjectConfig object

#### Scenario: Config file does not exist
- **WHEN** `ovsespec/config.yaml` does not exist
- **THEN** system returns null without error

#### Scenario: Config file has invalid YAML syntax
- **WHEN** `ovsespec/config.yaml` contains malformed YAML
- **THEN** system logs a warning message and returns null

#### Scenario: Config file has valid YAML but invalid schema
- **WHEN** `ovsespec/config.yaml` contains valid YAML that fails Zod schema validation
...
- **WHEN** `ovsespec/config.yml` exists and `ovsespec/config.yaml` does not exist
- **THEN** system reads from `ovsespec/config.yml`

#### Scenario: Both .yaml and .yml exist
- **WHEN** both `ovsespec/config.yaml` and `ovsespec/config.yml` exist
- **THEN** system prefers `ovsespec/config.yaml`
...
- **WHEN** config file has syntax errors and user runs `ovsespec new change`
