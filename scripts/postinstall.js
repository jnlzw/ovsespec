 ...
 * `ovsespec completion install` explicitly. This script only
 * prints a one-line tip after npm install.
 *
 * The tip is suppressed when:
 * - CI=true environment variable is set
 * - OVSESPEC_NO_COMPLETIONS=1 environment variable is set
...
function shouldSkipInstallation() {
  ...
  if (process.env.OVSESPEC_NO_COMPLETIONS === '1') {
    return { skip: true, reason: 'OVSESPEC_NO_COMPLETIONS=1 set' };
  }
...
async function main() {
  try {
    ...
    console.log(`\nTip: Run 'ovsespec completion install' for shell completions`);
  } catch (error) {
