...
OVSX is now the standard workflow for OvseSpec.

It's a **fluid, iterative workflow** for OvseSpec changes. No more rigid phases — just actions you can take anytime.

## Why This Exists

The legacy OvseSpec workflow works, but it's **locked down**:
...
**OVSX opens it up.** Now anyone can:
...
- **OvseSpec contributors** — experiment with new approaches without releases

We're all still learning what works best. OVSX lets us learn together.
...
```bash
# Make sure you have ovsespec installed — skills are automatically generated
ovsespec init
```

This creates skills in `.claude/skills/` (or equivalent) that AI coding assistants auto-detect.

By default, OvseSpec uses the `core` workflow profile (`propose`, `explore`, `apply`, `sync`, `archive`). If you want the expanded workflow commands (`new`, `continue`, `ff`, `verify`, `bulk-archive`, `onboard`), configure them with `ovsespec config profile` and apply with `ovsespec update`.

During setup, you'll be prompted to create a **project config** (`ovsespec/config.yaml`). This is optional but recommended.
...
Config is created during `ovsespec init`, or manually:

```yaml
# ovsespec/config.yaml
schema: spec-driven
...
2. Change metadata (`.ovsespec.yaml` in change directory)
...
- Run `ovsespec schemas --json` to see artifact IDs for each schema

**Config not being applied:**
- Ensure file is at `ovsespec/config.yaml` (not `.yml`)
...
|---------|--------------|
| `/ovsx:propose` | Create a change and generate planning artifacts in one step (default quick path) |
| `/ovsx:explore` | Think through ideas, investigate problems, clarify requirements |
| `/ovsx:new` | Start a new change scaffold (expanded workflow) |
| `/ovsx:continue` | Create the next artifact (expanded workflow) |
| `/ovsx:ff` | Fast-forward planning artifacts (expanded workflow) |
| `/ovsx:apply` | Implement tasks, updating artifacts as needed |
| `/ovsx:verify` | Validate implementation against artifacts (expanded workflow) |
| `/ovsx:sync` | Sync delta specs to main (default workflow, optional) |
| `/ovsx:archive` | Archive when done |
| `/ovsx:bulk-archive` | Archive multiple completed changes (expanded workflow) |
| `/ovsx:onboard` | Guided walkthrough of an end-to-end change (expanded workflow) |
...
/ovsx:explore
```
Think through ideas, investigate problems, compare options. No structure required - just a thinking partner. When insights crystallize, transition to `/ovsx:propose` (default) or `/ovsx:new`/`/ovsx:ff` (expanded).
...
```text
/ovsx:new        # scaffold only
/ovsx:continue   # create one artifact at a time
/ovsx:ff         # create all planning artifacts at once
```
...
/ovsx:apply
```
Works through tasks, checking them off as you go. If you're juggling multiple changes, you can run `/ovsx:apply <name>`; otherwise it should infer from the conversation and prompt you to choose if it can't tell.

### Finish up
```
/ovsx:archive   # Move to archive when done (prompts to sync specs if needed)
```
...
| | Legacy (`/ovsespec:proposal`) | OVSX (`/ovsx:*`) |
|---|---|---|
...
**The key insight:** work isn't linear. OVSX stops pretending it is.

## Architecture Deep Dive

This section explains how OVSX works under the hood and how it compares to the legacy workflow.
...
│         │                     │                     │                       │
│         ▼                     ▼                     ▼                       │
│   /ovsespec:proposal   /ovsespec:apply      /ovsespec:archive              │
│                                                                             │
...
│                    │                                                        │
│                    ▼                                                        │
│   Generated Command Files (.claude/commands/ovsespec/*.md)                  │
│                                                                             │
...
**OVSX** uses external schemas and a dependency graph engine:
...
│                    │                                                        │
│                    ▼                                                        │
│   Skill Files (.claude/skills/ovsespec-*/SKILL.md)                          │
│                                                                             │
...
**OVSX** — agent queries for rich context:
...
```
  ...
  │  ┌────────────────────────────────────────────────────────────────────┐  │
  │  │  $ ovsespec status --change "add-auth" --json                      │  │
  │  │                                                                    │  │
  │  │  {                                                                 │  │
  │  │    "artifacts": [                                                  │  │
  ...
  │  ┌────────────────────────────────────────────────────────────────────┐  │
  │  │  $ ovsespec instructions specs --change "add-auth" --json          │  │
  │  │                                                                    │  │
  │  │  {                                                                 │  │
...
**OVSX** — natural iteration:

```
  /ovsx:new ───► /ovsx:continue ───► /ovsx:apply ───► /ovsx:archive
      │                │                  │
      │                │                  ├── "The design is wrong"
      │                │                  │
      │                │                  ▼
...
ovsespec schema which my-workflow
```

Schemas are stored in `ovsespec/schemas/` (project-local, version controlled) or `~/.local/share/ovsespec/schemas/` (user global).

**Schema structure:**
```
ovsespec/schemas/research-first/
├── schema.yaml
└── templates/
    ├── research.md
    ├── proposal.md
...
- Use `/ovsx:explore` to think through an idea before committing to a change
- `/ovsx:ff` when you know what you want, `/ovsx:continue` when exploring
- During `/ovsx:apply`, if something's wrong — fix the artifact, then continue
- Tasks track progress via checkboxes in `tasks.md`
- Check status anytime: `ovsespec status --change "name"`

## Feedback

This is rough. That's intentional — we're learning what works.

Found a bug? Have ideas? Join us on [Discord](https://discord.gg/YctCnvvshC) or open an issue on [GitHub](https://github.com/Fission-AI/ovsespec/issues).
