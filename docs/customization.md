...
OvseSpec provides three levels of customization:
...
The `ovsespec/config.yaml` file is the easiest way to customize OvseSpec for your team. It lets you:
...
```bash
# Without config
ovsespec new change my-feature --schema spec-driven

# With config - schema is automatic
ovsespec new change my-feature
```
...
When OvseSpec needs a schema, it checks in this order:

1. CLI flag: `--schema <name>`
2. Change metadata (`.ovsespec.yaml` in the change folder)
...
When project config isn't enough, create your own schema with a completely custom workflow. Custom schemas live in your project's `ovsespec/schemas/` directory and are version-controlled with your code.
...
This copies the entire `spec-driven` schema to `ovsespec/schemas/my-workflow/` where you can edit it freely.

**What you get:**

```text
ovsespec/schemas/my-workflow/
...
```yaml
# ovsespec/schemas/my-workflow/schema.yaml
name: my-workflow
version: 1
...
```bash
# Specify on command
ovsespec new change feature --schema my-workflow
...
Schema: my-workflow
Source: project
Path: /path/to/project/ovsespec/schemas/my-workflow
```

---

> **Note:** OvseSpec also supports user-level schemas at `~/.local/share/ovsespec/schemas/` for sharing across projects, but project-level schemas in `ovsespec/schemas/` are recommended since they're version-controlled with your code.
...
```yaml
# ovsespec/schemas/rapid/schema.yaml
name: rapid
version: 1
...
```bash
ovsespec schema fork spec-driven with-review
```
...
OvseSpec also supports community-maintained schemas distributed via standalone repositories. These provide opinionated workflows that integrate OvseSpec with other tools or systems, similar to how [github/spec-kit's community extension catalog](https://github.com/github/spec-kit/tree/main/extensions) works for spec-kit.

Community schemas are not vendored into OvseSpec core — they live in their own repositories with their own release cadence. To use one, copy the schema bundle into your project's `ovsespec/schemas/<schema-name>/` directory (each repo's README has install instructions).

| Schema | Maintainer | Repository | Description |
|--------|-----------|-----------|-------------|
| `superpowers-bridge` | @JiangWay | [JiangWay/ovsespec-schemas](https://github.com/JiangWay/ovsespec-schemas/tree/main/superpowers-bridge) | Integrates OvseSpec's artifact governance with [obra/superpowers](https://github.com/obra/superpowers) execution skills (brainstorming, writing-plans, TDD via subagents, code review, finishing). Adds an evidence-first `retrospective` artifact filling a gap Superpowers does not natively cover. |
