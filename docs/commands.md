...
This is the reference for OvseSpec's slash commands. These commands are invoked in your AI coding assistant's chat interface (e.g., Claude Code, Cursor, Windsurf).
...
| `/ovsx:apply` | Implement tasks from the change |
...
| `/ovsx:continue` | Create the next artifact based on dependencies |
| `/ovsx:ff` | Fast-forward: create all planning artifacts at once |
| `/ovsx:verify` | Validate implementation matches artifacts |
| `/ovsx:bulk-archive` | Archive multiple changes at once |
| `/ovsx:onboard` | Guided tutorial through the complete workflow |

The default global profile is `core`. To enable expanded workflow commands, run `ovsespec config profile`, select workflows, then run `ovsespec update` in your project.
...
**Syntax:**
```text
/ovsx:propose [change-name-or-description]
```
...
- If you want step-by-step artifact control, enable expanded workflows and use `/ovsx:new` + `/ovsx:continue`
...
- Can transition to `/ovsx:propose` (default) or `/ovsx:new` (expanded workflow) when insights crystallize
...
AI:  Ready when you are. Run /ovsx:propose add-jwt-auth to begin.
```
...
Start a new change scaffold. Creates the change folder and waits for you to generate artifacts with `/ovsx:continue` or `/ovsx:ff`.

This command is part of the expanded workflow set (not included in the default `core` profile).

**Syntax:**
```
/ovsx:new [change-name] [--schema <schema-name>]
```
...
**What it does:**
- Creates `ovsespec/changes/<change-name>/` directory
- Creates `.ovsespec.yaml` metadata file in the change folder
- Shows the first artifact template ready for creation
- Prompts for change name and schema if not provided

**What it creates:**
```
ovsespec/changes/<change-name>/
└── .ovsespec.yaml    # Change metadata (schema, created date)
```

**Example:**
```
You: /ovsx:new add-dark-mode

AI:  Created ovsespec/changes/add-dark-mode/
     Schema: spec-driven

     Ready to create: proposal
     Use /ovsx:continue to create it, or /ovsx:ff to create all artifacts.
```

**Tips:**
- Use descriptive names: `add-feature`, `fix-bug`, `refactor-module`
- Avoid generic names like `update`, `changes`, `wip`
- Schema can also be set in project config (`ovsespec/config.yaml`)
...
AI:  Change: add-dark-mode
     ...
     ✓ Created ovsespec/changes/add-dark-mode/specs/ui/spec.md

     Now available: tasks
     Run /ovsx:continue to create the next artifact.
```
...
AI:  Fast-forwarding add-dark-mode...
     ...
     Ready for implementation. Run /ovsx:apply to begin.
```

**Tips:**
- Use when you have a clear picture of what you're building
- Faster than `/ovsx:continue` for straightforward changes
...
- Merges changes into main `ovsespec/specs/` directory
...
AI:  Syncing add-dark-mode delta specs...
     ...
     Reading: ovsespec/changes/add-dark-mode/specs/ui/spec.md
     Target:  ovsespec/specs/ui/spec.md
     ...
     Merging...
     ✓ ovsespec/specs/ui/spec.md updated

     Change remains active. Run /ovsx:archive when ready to complete.
```
...
- Moves change folder to `ovsespec/changes/archive/YYYY-MM-DD-<name>/`
...
AI:  ✓ Synced specs to ovsespec/specs/ui/spec.md
     ✓ Moved to ovsespec/changes/archive/2025-01-24-add-dark-mode/
...
- Use `/ovsx:verify` first to catch issues
...
**Syntax:**
```
/ovsx:bulk-archive [change-names...]
```
...
Guided onboarding through the complete OvseSpec workflow. An interactive tutorial using your actual codebase.
...
3. Creating a change (`/ovsx:new`)
4. Writing the proposal
5. Creating specs
6. Writing the design
7. Creating tasks
8. Implementing tasks (`/ovsx:apply`)
...
| Tool | Syntax Example |
|------|----------------|
| Claude Code | `/ovsx:propose`, `/ovsx:apply` |
| Cursor | `/ovsx-propose`, `/ovsx-apply` |
| Windsurf | `/ovsx-propose`, `/ovsx-apply` |
| Copilot (IDE) | `/ovsx-propose`, `/ovsx-apply` |
| Kimi CLI | Skill-based invocations such as `/skill:ovsespec-propose`, `/skill:ovsespec-apply-change` (no generated `ovsx-*` command files) |
| Trae | Skill-based invocations such as `/ovsespec-propose`, `/ovsespec-apply-change` (no generated `ovsx-*` command files) |
...
These commands use the older "all-at-once" workflow. They still work but OVSX commands are recommended.
...
**Migrating to OVSX:**
Legacy changes can be continued with OVSX commands. The artifact structure is compatible.
...
**Solutions:**
- Specify the change name explicitly: `/ovsx:apply add-dark-mode`
- Check that the change folder exists: `ovsespec list`
...
**Solutions:**
- Run `ovsespec status --change <name>` to see what's blocking
...
**Solutions:**
- List available schemas: `ovsespec schemas`
- Check spelling of schema name
- Create the schema if it's custom: `ovsespec schema init <name>`

### Commands not recognized

The AI tool doesn't recognize OvseSpec commands.

**Solutions:**
- Ensure OvseSpec is initialized: `ovsespec init`
- Regenerate skills: `ovsespec update`
...
**Solutions:**
- Add project context in `ovsespec/config.yaml`
- Add per-artifact rules for specific guidance
- Provide more detail in your change description
- Use `/ovsx:continue` instead of `/ovsx:ff` for more control
